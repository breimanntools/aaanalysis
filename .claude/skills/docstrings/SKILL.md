...
7. **Verify every citation — never invent one.** Valid only if `Key` is defined in `references.rst` *and* the work describes this symbol. **Citations are the exception** (core algorithm classes); utilities/loaders/preprocessors carry **none** — don't reflexively add `[Breimann25]_`. When unsure, omit.
