...
- **Migrate dev workflow from Poetry to uv in the same PR.** Independent concern. Tracked in `docs/v2-followups.md`. End-user `pip install aaanalysis` and `uv add aaanalysis` both work today regardless of which dev tool the project uses internally.
