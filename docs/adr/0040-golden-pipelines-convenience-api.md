...
The fix is a second, opt-in API — `import aaanalysis.pipe as ap` — of **golden
...
- **Means** are producers of an *input* to an End (reliable negatives, a reduced scale
  ...
  standalone `ap` producer only on genuine standalone need, and is never *both* a flag
...
bare value). A modelling End returns `(model, df_eval)`. `ap` **adopts whatever plot
return-contract #133 settles** rather than inventing its own — the plot return shape
is extracted at the wrapper boundary, not hard-coded deep in `ap`.
...
like `matplotlib.pyplot`: a dedicated **API-reference page** (autosummary of `ap.*`),
a **narrative "Golden pipelines" guide** stating the two-interface model (primitives
for control, `ap` for quick results), each pipeline cross-linked to the primitives it
...
`import aaanalysis.pipe as ap`) until it stabilizes — so it is not yet on the
...
- `ap`'s plot return is coupled to #133; the two must land their plot-return shape in the
  same release.
...
- The sklearn `SequenceFeatureTransformer` (a `feature_engineering` interop class, not an
  `ap` pipeline) — tracked in #241, decided separately.
