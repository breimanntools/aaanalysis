...
| **Packaging** | built wheel installs clean; full public API imports; bundled `_data` loads | `.github/workflows/packaging.yml` ("Packaging", push + PR to master): builds sdist + wheel with `python -m build --sdist --wheel` (from source — the default sdist→wheel chain fails, the sdist omits the Cython `.pyx`), installs the **wheel** into a fresh base-deps-only venv, and runs `tests/_check_public_api_packaged.py` — every `__all__` symbol imports, `pro`/`dev` symbols degrade to `missing_feature_stub`, `load_*` reads `_data`. **Ubuntu py3.10 + 3.14**, one wheel built per version (ABI-specific). Catches missing package-data / broken re-exports the editable dev matrix cannot — ADR-0060. |
