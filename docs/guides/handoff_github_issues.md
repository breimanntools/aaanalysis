...
_Generated by the `github-issue-handoff` skill on 2026-06-06; last refreshed 2026-06-11 (#37 AAMut/SeqMut DONE/merged — protein-design chain #57/#59/#60 UNBLOCKED; #32 feature pruning + #84 coverage gate DONE/merged; open count 60 → 57). Prior refresh 2026-06-10 (#61 CLOSED + P8 "Prediction" protocol; +4 docs-architecture issues #106–#109). Regenerate:_
...
- **v1.1** (~13 + board) — **release-readiness: docs onramp + quick wins + API trust**
  ...
  #82, ~~#84~~ (param/branch coverage — **DONE/merged `32847f06`**). **+ "key next steps" from the GitHub Project board** (board now fully synced —
...
- **v1.2** (3 open; #61 + #58 closed; #37 merged) — **protein design** (moved up from v1.3;
  "important and soon"): ~~#37~~ (AAmut/SeqMut anchor — **DONE/merged `33248426`+`0c090bff`,
  PRs #112+#117; issue kept open for review; the chain gate has LANDED**), ~~#61~~
  (multi-class/regression — **merged to master `c4fc3c72`, closed**), #59 (multi-objective
  scoring — interpretable, no black-box; **kept in core**), **#57** (the "feature-importance →
  ranked minimal mutation set" primitive — **now unblocked**; **#58 merged in and closed as
  duplicate**; campaign-looping → ProtXplain), #16 (uncertainty estimations — CPP-feature
  primitive, feeds #60).
- **v1.3** (13 open; #32 merged) — **remaining important: core CPP feature & evaluation story**
  (the interpretable-prediction backbone — feature selection/export/normalization, CPP
  ...
  (normalization), ~~#32~~ (feature selection — **DONE/merged**), #33 (export formats),
  #36 (scale selection), #43 (AAclust), #87 (CPP strategy preset), #88 (security audit),
  #89 (residue-pair features), #91 (model eval: repeated-CV + bootstrap CIs + paired ΔMCC),
  #93 (learning-curve utility).
...
Dependency order (from the lanes below): ~~#18 before #61~~ (#18 done; **#61 done/closed**), ~~**#37 before #57/#59**~~ (**#37 done/merged — protein-design chain UNBLOCKED**). Milestones complement
...
buckets, and closed **#58** (dup of #57). The GitHub Project **"AAanalysis"** Kanban was
fully synced (all open issues carded at that point) and driven by a 3-tier `prio:` scheme — see
below. _Since then (2026-06-11) #37 / #32 / #84 landed on master → open count is now **57**;
move their cards to Done._ Remaining doc chore: reconcile the per-issue verdict-table
`Milestone` column further down._
...
Every open issue carries **exactly one** `prio:` label. Current split — **8 / 14 / 35** (57 open):

- **prio:1 (8 — To Do):** #16, #21, #57, #59, #74, #81, #91, #106.
  = v1.1 release-readiness **4** (#21 golden-path tutorial · #74 CHANGELOG/deprecation ·
  #81 cheat sheet · #106 docs-architecture epic) + v1.2 protein-design **3** (#16, #57, #59)
  + v1.3 trust/eval **1** (#91 model eval). (#32 feature selection, #37 AAMut/SeqMut, #84
  param/branch coverage all DONE/merged — see Snapshot.)
- **prio:2 (14 — headline-next, Backlog):** v1.1 #20, #35, #80, #107, #109 · v1.3 #28,
  #29, #33, #43, #88 · v1.4 #22, #40, #56, #60.
- **prio:3 (35 — convenience / extensions / research / heavy-deferred, Backlog):** the
...
extensions, research-y, or heavy-and-deferred.** Caveats: **#57/#59/#60 are UNBLOCKED — #37
landed** (merged to master; the gate for the protein-design chain has cleared); **v1.2 is
entirely prio:1** by request.

## Snapshot
- **Open issues: 57 · closed: 33.**
- **#37 — ☑️ DONE / merged to `master` (`33248426` `feat(protein_design): finish AAMut & SeqMut — CPP-guided mutation/ΔCPP module (#37)` + `0c090bff` notebook render fix; PRs #112 + #117 merged). Issue intentionally LEFT OPEN for author review, but implementation is complete on master.** This was the GATE for the protein-design chain #57/#59/#60 — that gate has now **LANDED**, so the chain is **UNBLOCKED**.
- **#32 — ☑️ DONE / merged (`49436417` + `cf4f7708` + `e7bba229`) — model-free feature pruning (variance + correlation).** Completes the open half of #32 (the model-based `TreeModel.select_features` half shipped earlier).
- **#84 — ☑️ DONE / merged (`32847f06`) — parameter-coverage meta-test + branch-coverage gate.**
...
|---|---|---|---|---|
| **57** | 1 | C — `protein_design/` (AAMut/SeqMut) | **now UNBLOCKED — #37 landed**; ML-guided directed evolution builds on the merged `SeqMut.suggest`/ΔCPP engine; separate subsystem, parallel-safe | feature-importance → ranked minimal mutation set; how it consumes the shipped ΔCPP scoring; campaign-looping → ProtXplain |
...
| **106** *(or **21**)* | 2 | E — docs-architecture epic | prio:1 docs onramp; split into children (#107/#108/#109) before executing; coordinate with #35/#21/#80/#81 | section nav + landing routing + API stable/experimental split; #21 hosts the concept-overview page + class table |

With **#17/#18/#37 landed** (and **#61/#32/#84 done**), the next
prio:1 picks are **#57** (Lane C, ML-guided directed evolution — now unblocked, builds on the
merged AAMut/SeqMut + ΔCPP engine), **#91** (Lane G, model eval), and the docs onramp
**#106/#21** (Lane E), which are genuinely parallel (different subsystems, no shared files).
The Lane-A tail is the schema *consumers* (**#29** normalization, **#33** export, **#26**
ProtXplain), which build on the frozen `LIST_COLS_FEAT` + `sort_cols_feat` contract. The small
quick win (**#24** sklearn wrapper) remains an alternative Lane D/E momentum task.
...
7. ☑️ **#37 — DONE / merged** (`33248426` + `0c090bff`; PRs #112 + #117). AAMut/SeqMut finished —
   CPP-guided mutation/ΔCPP module. **Issue kept open for author review, but on master.** This
   **unblocked the protein-design chain**, which now starts at **#57**. **Lane C.**
8. **#57** — ML-guided directed evolution (prio:1; **now unblocked — #37 landed**). Feature-importance
   → ranked minimal mutation set, building on the merged `SeqMut.suggest`/ΔCPP engine. **Lane C**,
   serialize ahead of #59 → #60.
9. **#91** — model evaluation & comparison (prio:1). Self-contained (reuses `aaanalysis.metrics` +
   `comp_bootstrap_ci`); first settle helpers vs `Tool`-class+Plot vs hybrid. **Lane G**; #93
   (learning-curve) builds on it.
10. Quick wins (small, independent): **#24, #33, #29** (#29/#33 now unblocked — #18 done; **#32
    variance/correlation filtering DONE/merged**).
11. Docs: **#106 (split into #107/#108/#109 first), #21, #43, #69 (retarget first), #20** (~~#90~~ done → P7 feature-selection protocol).
12. Revisit/deferred clusters: structure/conservation (#40/#64/#65/#42), embedding (#22/#23/#47),
...
- **Lane C — Protein design (SERIALIZE internally):** ~~#37~~ (✅ done/merged), ~~#58~~ (closed,
  dup of #57) → **{#57 → #59 → #60}**. Build on the merged AAMut/SeqMut + ΔCPP mutation workflow;
  share `protein_design/`.
...
| Protein design | ~~#37~~ (done), ~~#58~~ (closed) → #57 → #59 → #60 | #57 | shared mutation API in `protein_design/`; #37 gate landed |
...
- **#37** `33248426` + `0c090bff` (AAMut/SeqMut finished — CPP-guided mutation/ΔCPP module; PRs #112 + #117; **impl complete on master, issue kept OPEN for author review**) ·
  **#32** `49436417` + `cf4f7708` + `e7bba229` (model-free feature pruning — variance + correlation) ·
  **#84** `32847f06` (parameter-coverage meta-test + branch-coverage gate) ·
...
| 57 | 1 | 🔄 | Fits; **UNBLOCKED (#37 landed)** | No | Top-k residue selection from feature importance; reduced libraries; build on the merged AAMut/SeqMut + ΔCPP engine. **First in Lane C now.** |
| 58 | 1 | ☑️ | Fits; **CLOSED** (dup of #57) | Yes | Residue-level ΔCPP scoring / minimal mutation sets merged into #57. |
| 59 | 1 | 🔄 | Fits; **UNBLOCKED (#37 landed)**; after #57 | No | Weighted/Pareto mutation ranking. "No black-box" constraint = keep interpretable. |
| 60 | 1 | 🔄 | Fits; **UNBLOCKED (#37 landed)**; after #59 | No | Uncertainty/diversity candidate selection over CPP feature space. |
| 37 | 2 | ☑️ | Fits; **DONE / merged** (`33248426` + `0c090bff`; PRs #112 + #117). Issue kept OPEN for author review | Yes | DONE: finished AAMut/SeqMut — CPP-guided mutation/ΔCPP module. Was the **gate for #57–60**; gate has now landed → chain unblocked. Lane C now starts at #57. |
...
| 32 | 1 | ☑️ | Small; fits; **DONE / merged** (`49436417` + `cf4f7708` + `e7bba229`) | Yes | DONE in two halves: **model-based** post-fit selection `TreeModel.select_features` (`top_k`/`threshold`/`frequency`, ADR-0023, `325c28ca`) **+ model-free variance/correlation pruning** (`49436417` + `cf4f7708` + `e7bba229`). Both halves now on master. |
...
| 41 | 3 | ☑️ | **Closed** (2026-06-06 triage) | Yes | Split → **#84 (param + branch coverage — now ☑️ DONE/merged `32847f06`)**; integration/e2e stay v2-deferred (sharp-edges). |
