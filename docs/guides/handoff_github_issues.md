...
_Generated by the `github-issue-handoff` skill on 2026-06-06; last refreshed 2026-06-10 (#61 re-implemented on `feat/issue-61` off current master — label helpers + row-subset ergonomics solved in-method; awaiting merge). Regenerate:_
...
- **#61 — RE-IMPLEMENTED locally on `feat/issue-61` (off current master); green; awaiting merge — multi-class/regression.**
  Earlier history: merged (PR #97 → `186110a8`), **reverted** (`c41018d5`), re-proposed in PR #99
  (`7f925ceb`) which **never actually landed** (a handoff note claimed it did; the methods were
  absent from master). On **2026-06-10** the work was re-integrated cleanly onto branch
  `feat/issue-61` off current master and committed (`c4fc3c72` feature + `ac86c867` versionadded
  audit). Final shape (settled via `/grill-with-docs`, per memory `project_issue61_reframed`):
  **label helpers on `SequenceFeature`** — `get_labels_ovr` / `get_labels_ovo` /
  `get_labels_quantile` + `get_labels_tiered` (fixed positives, stepwise-lowered negative quantile
  cuts) — **not** `fit_multiclass`/`fit_regression`. The in-house `get_df_parts_reference` was
  **removed**: reference parts come from `AAWindowSampler` + **`get_df_parts_from_windows`**
  (per-part priors, e.g. coil JMD / α-helix TMD). **The "part problem" is now SOLVED in-method:**
  `get_labels_ovo` / `get_labels_tiered` take the value source (`df_parts` **and/or**
  `dict_num_parts`) and return per group the **row-matched copy + binary labels** (mask internal
  only), ready to drop into `CPP.run` / `CPP.run_num` — so **no separate "part problem" issue is
  needed**. Tests rewritten to the house template (130 tests; full SF suite 329 green); 0 docstring
  defects; 5 example notebooks with executed outputs; `versionadded` now on all SF methods. **Next:
  merge `feat/issue-61` → master.** Lane A core = #17/#18 done; **#61 implemented, awaiting merge.**
...
With **#17/#18 landed** (and **#61 implemented on `feat/issue-61`, awaiting merge**), the next
...
4. ☑️ **#18 — DONE** (`0e561566`). Standardized CPP output schema as a *stabilize + document*
   ...
   #29/#33/#26/#61.** **Lane A.**
5. ☑️ **#61 — RE-IMPLEMENTED on `feat/issue-61` (off current master); green; awaiting merge.** PR #99
   never landed; re-integrated 2026-06-10 (`c4fc3c72` + `ac86c867`). Final shape — **label helpers on
...
- **Lane A — CPP core/schema:** ~~#17~~ (done), ~~#18~~ (done), **#61** (implemented on
  `feat/issue-61`, awaiting merge) → schema **consumers** #29 / #33 / #26 (build on `LIST_COLS_FEAT` +
...
| Output schema | ~~#18~~ (done), #61 (`feat/issue-61`, awaiting merge) → #29, #33, #26 | #29 | #18 froze the contract; remaining consumers serialize behind it in Lane A (build on `LIST_COLS_FEAT`/`sort_cols_feat`) |
...
| 61 | 1 | ☑️ | Fits core; **re-implemented on `feat/issue-61` (off current master), green, awaiting merge** | Implemented (branch) | PR #99 never landed; re-integrated 2026-06-10 (`c4fc3c72` + `ac86c867`). **Label helpers on `SequenceFeature`** (`get_labels_ovr`/`ovo`/`quantile`/`tiered`), **not** `CPP.fit_*`; `get_df_parts_reference` **removed** → `AAWindowSampler` + `get_df_parts_from_windows`. **Row-subset ergonomics SOLVED in-method** — `ovo`/`tiered` take `df_parts`/`dict_num_parts`, return row-matched copies for `CPP.run`/`run_num` (no "part problem" issue needed). 130 tests, full SF suite green, `versionadded` on all SF methods. **Next: merge → master.** |
