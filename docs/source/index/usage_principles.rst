...
below — from the external data sources through the two CPP entry points to the feature
matrix ``X`` and the model, explanation, and design wrappers
(`open the full map <../_static/dataflow_map.html>`_). Either amino acid scales or
protein embeddings can serve as the numerical representation of amino acids: ``CPP.run``
consumes amino acid scales, while ``CPP.run_num`` consumes numeric values such as
protein-language-model embeddings, which can be created via
