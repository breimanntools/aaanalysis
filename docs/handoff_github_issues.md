...
_Generated by the `github-issue-handoff` skill on 2026-06-06; last refreshed 2026-06-08 (#18 closed). Regenerate:_
...
Respect the dependency order from the lanes below: ~~#18 before #61~~ (**#18 done** — #61
now unblocked), **#37 before #57–60** (protein-design chain). Milestones complement the `prio:` labels —
...
- **#18 — CLOSED (`0e561566`, 2026-06-08; green CI) — the prio:1 schema keystone.** Settled
  ...
  **`ut.split_feat_id` / `ut.join_feat_id`** replacing ~15 duplicated inline `.split("-")`
  ...
  lives in docstring + glossary). **Unblocks #61 (now the next prio:1 in Lane A), #29, #33, #26**
...
|---|---|---|---|---|
| **61** | 1 | A — `feature_engineering/_cpp.py` + output | prio:1; **now unblocked** (#18 landed the schema it consumes); best-specified feature | reference-sequence generation + `fit_multiclass`/`fit_regression`; thread `random_state`; how the new outputs slot into `LIST_COLS_FEAT` / `sort_cols_feat` (build on the #18 contract, don't re-derive) |
...
With ~~#18~~ now landed, **#61 is the next prio:1** and owns **Lane A** (it ripples into the same
`_cpp.py`/output area #18 touched — keep one owner). **#37** (Lane C) and **#91** (Lane G) are the
genuinely parallel companions — different subsystems, no shared files, safe in three separate
sessions. The small quick win (**#24** sklearn wrapper, or **#32**-remainder variance/correlation
filtering) remains an alternative Lane D/E momentum task.
...
4. ☑️ **#18 — DONE** (`0e561566`). Standardized CPP output schema as a *stabilize + document*
   ...
   #29/#33/#26/#61.** **Lane A.**
5. **#61** — multi-class/regression via random reference (prio:1). **Now unblocked** (#18
   landed). Best-specified feature; sizable but self-contained. Build outputs on the #18
...
- **Lane A — CPP core/schema:** ~~#17~~ (done), ~~#18~~ (done) → **#61** (multi-class/regression
  builds on the #18 schema; touches `_cpp.py`/output — keep one owner).
...
| Output schema | ~~#18~~ (done) → #61, #29, #33, #26 | #61 | #18 froze the contract; consumers now serialize behind it in Lane A (build on `LIST_COLS_FEAT`/`sort_cols_feat`) |
