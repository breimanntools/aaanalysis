...
_Generated by the `github-issue-handoff` skill on 2026-06-06; last refreshed 2026-06-09 (CPP.simplify shipped; #61 merged then reverted — re-review in PR #99). Regenerate:_
...
Respect the dependency order from the lanes below: ~~#18 before #61~~ (#18 done; **#61 merged
then reverted off master — pending re-review in PR #99**), **#37 before #57–60** (protein-design chain). Milestones complement the `prio:` labels —
they answer "additive → which v1.x minor" vs "breaking/heavy → v2.X".

## Snapshot
- **Open issues: 57 · closed: 29.**
- **#61 — REVERTED off master; OPEN again (PR #99) — multi-class/regression.** It was merged
  (PR #97 → `186110a8`) then **reverted from master** (`c41018d5`) at the maintainer's request;
  the issue was reopened and the work re-proposed in **open PR #99** (PR #97 could not be reopened
  once merged). The **approach is decided** (per memory `project_issue61_reframed`): **label
  helpers on `SequenceFeature`** — `get_labels_ovr` / `get_labels_ovo` / `get_labels_quantile` +
  df_parts-level reference generation — **not** `fit_multiclass`/`fit_regression`; OvO /
  banded-regression deferred to a future "part problem" issue. **Not on master** — re-merge is a
  pending decision (PR #99 is green-equivalent to the original branch). Lane A core = #17/#18 done;
  #61 pending.
...
With **#17/#18 landed** (and **#61 reverted off master — pending re-merge via PR #99**), the next
prio:1 picks are **re-merging #61** (review PR #99) plus **#37** (Lane C, the protein-design gate)
...
4. ☑️ **#18 — DONE** (`0e561566`). Standardized CPP output schema as a *stabilize + document*
   ...
   #29/#33/#26/#61.** **Lane A.**
5. 🔄 **#61 — REVERTED off master; OPEN (PR #99).** Merged (PR #97 → `186110a8`) then **reverted**
   (`c41018d5`) at the maintainer's request; reopened + re-proposed in **PR #99**. Approach decided
   — **label helpers on `SequenceFeature`** (`get_labels_ovr`/`ovo`/`quantile`) + df_parts-level
   reference generation, **not** CPP `fit_multiclass`/`fit_regression`; OvO / banded-regression
   deferred to a future "part problem" issue. **Next step = re-review/re-merge PR #99.** ~~Original
   framing below superseded:~~ #18
   contract (`LIST_COLS_FEAT` + `sort_cols_feat`). **Lane A** (one owner with the `_cpp.py`/output area).
...
| 61 | 1 | 🔄 | Fits core; **reverted off master — open PR #99** | Partial (code exists on branch) | Merged (PR #97 → `186110a8`) then reverted (`c41018d5`) at maintainer's request; reopened, re-proposed in **PR #99**. Approach decided: **label helpers on `SequenceFeature`** (`get_labels_ovr`/`ovo`/`quantile`) + df_parts-level reference, **not** `CPP.fit_multiclass`/`fit_regression`. OvO / banded-regression deferred. **Next: re-review/re-merge PR #99.** |
