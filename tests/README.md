...
_Empty._ The previously-inventoried private dead functions and the orphan module
`seq_analysis_pro/_comp_seq_cons.py` have all been removed (each verified to have zero
call sites in `aaanalysis/` and `tests/` before removal), and the coverage exclusion for
the orphan module was dropped.
