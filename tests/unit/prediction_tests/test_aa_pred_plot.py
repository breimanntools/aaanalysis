...
class TestAAPredPlotPredictRanking:
    def test_returns_fig_ax(self):
        r = aa.AAPredPlot().predict(_df_rank(), kind="ranking")
        assert r.fig is not None and r.ax is not None

    def test_col_name_score(self):
        df = _df_rank().rename(columns={"name": "gene", "score": "p"})
        r = aa.AAPredPlot().predict(df, kind="ranking", col_name="gene", col_score="p")
        assert r.ax is not None

    def test_col_group_and_std(self):
        r = aa.AAPredPlot().predict(_df_rank(), kind="ranking", col_group="group", col_std="std")
        assert len(r.ax.patches) == 8

    def test_missing_column_raises(self):
        with pytest.raises(ValueError):
            aa.AAPredPlot().predict(_df_rank().drop(columns=["score"]), kind="ranking")

    def test_empty_raises(self):
        with pytest.raises(ValueError):
            aa.AAPredPlot().predict(_df_rank().iloc[0:0], kind="ranking")

    def test_top_n_and_ascending(self):
        r = aa.AAPredPlot().predict(_df_rank(), kind="ranking", top_n=3, ascending=True)
        assert len(r.ax.patches) == 3

    def test_cutoffs_and_colors(self):
        r = aa.AAPredPlot().predict(_df_rank(), kind="ranking", col_group="group",
                                    colors={"sub": "red", "non": "blue"}, cutoffs=(60,))
        assert any(line.get_linestyle() == "--" for line in r.ax.get_lines())

    def test_figsize_height_scales_with_items(self):
        h8 = aa.AAPredPlot().predict(_df_rank(8), kind="ranking").fig.get_size_inches()[1]
        h20 = aa.AAPredPlot().predict(_df_rank(20), kind="ranking").fig.get_size_inches()[1]
        assert h20 > h8

    def test_ax_xlabel_title(self):
        _, ax = plt.subplots()
        r = aa.AAPredPlot().predict(_df_rank(), kind="ranking", ax=ax, xlabel="score", title="t")
        assert r.ax is ax
...
class TestAAPredPlotPredictClustermap:
    def test_returns_fig_ax(self):
        r = aa.AAPredPlot().predict(_imp_data(), kind="clustermap")
        assert r.fig is not None and r.ax is not None

    def test_labels_and_names(self):
        r = aa.AAPredPlot().predict(_imp_data(), kind="clustermap", labels=np.array([1, 0] * 6),
    ...
    def test_min_samples_raises(self):
        with pytest.raises(ValueError):
            aa.AAPredPlot().predict(np.random.RandomState(0).rand(1, 5), kind="clustermap")

    def test_labels_length_mismatch_raises(self):
        with pytest.raises(ValueError):
            aa.AAPredPlot().predict(_imp_data(), kind="clustermap", labels=np.array([1, 0, 1]))

    def test_names_length_mismatch_raises(self):
        with pytest.raises(ValueError):
            aa.AAPredPlot().predict(_imp_data(), kind="clustermap", names=["a", "b"])

    def test_figsize_cbar_label_title(self):
        r = aa.AAPredPlot().predict(_imp_data(), kind="clustermap", figsize=(6, 6),
    ...
    def test_constant_row_does_not_crash(self):
        ...
        data = _imp_data()
        data[0] = 0.0
        r = aa.AAPredPlot().predict(data, kind="clustermap", labels=np.array([1, 0] * 6))
        assert r.ax is not None

    def test_string_labels_allowed(self):
        r = aa.AAPredPlot().predict(_imp_data(), kind="clustermap", labels=["sub", "non"] * 6)
        assert r.ax is not None
...
class TestAAPredPlotPredictHist:
    def test_returns_fig_ax(self):
        ...
        fig, ax = aa.AAPredPlot().predict(scores, kind="hist")
        assert fig is not None and ax is not None

    def test_labels_separated(self):
        scores, labels = _scores()
        fig, ax = aa.AAPredPlot().predict(scores, kind="hist", labels=labels)
        assert len(ax.get_legend().get_texts()) == 2

    def test_bins(self):
        scores, labels = _scores()
        fig, ax = aa.AAPredPlot().predict(scores, kind="hist", bins=5)
        assert ax is not None

    def test_thresholds(self):
        scores, labels = _scores()
        fig, ax = aa.AAPredPlot().predict(scores, kind="hist", thresholds=[0.4, 0.6])
    ...
    def test_labels_length_mismatch_raises(self):
        scores, labels = _scores()
        with pytest.raises(ValueError):
            aa.AAPredPlot().predict(scores, kind="hist", labels=labels[:-1])

    def test_xlabel_ylabel(self):
        scores, labels = _scores()
        fig, ax = aa.AAPredPlot().predict(scores, kind="hist", xlabel="Score", ylabel="Count")
        assert ax.get_xlabel() == "Score" and ax.get_ylabel() == "Count"

    def test_ax_figsize(self):
        scores, labels = _scores()
        fig, ax0 = plt.subplots()
        fig, ax = aa.AAPredPlot().predict(scores, kind="hist", ax=ax0, figsize=(6, 4))
        assert ax is ax0
...
class TestAAPredPlotPredictScatter:
    def test_returns_fig_ax(self):
        s1, labels = _scores(0)
        s2, _ = _scores(1)
        fig, ax = aa.AAPredPlot().predict(s1, kind="scatter", scores_y=s2)
        assert fig is not None and ax is not None

    def test_missing_scores_y_raises(self):
        s1, labels = _scores(0)
        with pytest.raises(ValueError):
            aa.AAPredPlot().predict(s1, kind="scatter")

    def test_scores_x_scores_y_length_mismatch(self):
        s1, labels = _scores(0)
        with pytest.raises(ValueError):
            aa.AAPredPlot().predict(s1, kind="scatter", scores_y=s1[:-1])

    def test_labels_color(self):
        s1, labels = _scores(0)
        s2, _ = _scores(1)
        fig, ax = aa.AAPredPlot().predict(s1, kind="scatter", scores_y=s2, labels=labels)
        assert ax.get_legend() is not None

    def test_diagonal_off(self):
        s1, labels = _scores(0)
        s2, _ = _scores(1)
        fig, ax = aa.AAPredPlot().predict(s1, kind="scatter", scores_y=s2, diagonal=False)
        assert ax is not None

    def test_marker_size(self):
        s1, labels = _scores(0)
        s2, _ = _scores(1)
        fig, ax = aa.AAPredPlot().predict(s1, kind="scatter", scores_y=s2, marker_size=50)
        assert ax is not None
    ...
    def test_xlabel_ylabel(self):
        s1, labels = _scores(0)
        s2, _ = _scores(1)
        fig, ax = aa.AAPredPlot().predict(s1, kind="scatter", scores_y=s2, xlabel="P1", ylabel="P2")
    ...
    def test_ax_figsize(self):
        ...
        s2, _ = _scores(1)
        fig, ax0 = plt.subplots()
        fig, ax = aa.AAPredPlot().predict(s1, kind="scatter", scores_y=s2, ax=ax0, figsize=(5, 5))
        assert ax is ax0
...
class TestAAPredPlotPredictCutoff:
    def test_returns_fig_ax(self):
        scores, labels = _scores()
        fig, ax = aa.AAPredPlot().predict(scores, kind="cutoff")
        assert fig is not None and ax is not None
    ...
    def test_n_steps(self):
        scores, labels = _scores()
        fig, ax = aa.AAPredPlot().predict(scores, kind="cutoff", n_steps=11)
        assert len(ax.get_lines()[0].get_ydata()) == 11

    def test_color(self):
        scores, labels = _scores()
        fig, ax = aa.AAPredPlot().predict(scores, kind="cutoff", color="#123456")
        assert ax is not None

    def test_thresholds(self):
        scores, labels = _scores()
        fig, ax = aa.AAPredPlot().predict(scores, kind="cutoff", thresholds=[0.5])
        assert any(l.get_linestyle() == "--" for l in ax.get_lines())

    def test_empty_scores_raises(self):
        with pytest.raises(ValueError):
            aa.AAPredPlot().predict(np.array([]), kind="cutoff")

    def test_xlabel_ylabel(self):
        scores, labels = _scores()
        fig, ax = aa.AAPredPlot().predict(scores, kind="cutoff", xlabel="Cutoff", ylabel="Percent")
        assert ax.get_xlabel() == "Cutoff" and ax.get_ylabel() == "Percent"

    def test_ax_figsize(self):
        scores, labels = _scores()
        fig, ax0 = plt.subplots()
        fig, ax = aa.AAPredPlot().predict(scores, kind="cutoff", ax=ax0, figsize=(6, 4))
        assert ax is ax0
