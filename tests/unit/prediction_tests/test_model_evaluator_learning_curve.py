...
class TestLearningCurve:
    ...
    def test_train_sizes_fractions(self, fracs):
        ...
        sizes = [int(np.floor(f * 32)) for f in fracs]
    ...
    def test_invalid_X(self):
        _, labels = _data()
        for X in [None, "X", [[1, 2], [3]], np.full((40, 3), np.nan)]:
            with pytest.raises(ValueError):
                _me().learning_curve(X, labels)

    def test_invalid_labels(self):
        X, _ = _data()
        for labels in [None, [0] * 40, [1, 2] * 20, [0, 1, 2, 3] * 10]:
            with pytest.raises(ValueError):
                _me().learning_curve(X, labels)
    ...
    def test_invalid_train_sizes_type(self):
        X, labels = _data()
        for train_sizes in ["0.5", 0.5, 8, ["a", "b"], [True, False], [0.5, 8], [0.5, None]]:
            with pytest.raises(ValueError):
    ...
    def test_invalid_train_sizes_int_range(self):
        X, labels = _data()
        for train_sizes in [[1, 8], [0, 8], [-4, 8], [8, 33], [8, 100]]:
            with pytest.raises(ValueError):
                _me().learning_curve(X, labels, train_sizes=train_sizes)
