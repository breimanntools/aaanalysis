...
class TestFeaturizerBinding:
    ...
    def test_predict_seq_without_df_feat_raises(self, fitted):
        ...
        with pytest.raises(ValueError):
            aapred.predict_seq(df_seq.head(3))


class TestPredictSeq:
    def test_columns(self, fitted):
        aapred, df_seq, _ = fitted
        df_pred = aapred.predict_seq(df_seq.head(5))
    ...
    def test_list_parts(self, fitted):
        aapred, df_seq, _ = fitted
        df_pred = aapred.predict_seq(df_seq.head(3), list_parts=["tmd", "jmd_n_tmd_n", "tmd_c_jmd_c"])
        assert len(df_pred) == 3

    def test_before_fit_raises(self, fitted):
        _, df_seq, df_feat = fitted
        with pytest.raises(ValueError):
            aa.AAPred(df_feat=df_feat).predict_seq(df_seq.head(3))


class TestPredictDomain:
    def test_columns(self, fitted):
        aapred, df_seq, _ = fitted
        df = aapred.predict_domain(df_seq[df_seq["entry"] == "P05067"], window=2)
    ...
    def test_exactly_one_best(self, fitted):
        aapred, df_seq, _ = fitted
        df = aapred.predict_domain(df_seq[df_seq["entry"] == "P05067"], window=3)
    ...
    def test_list_parts(self, fitted):
        aapred, df_seq, _ = fitted
        df = aapred.predict_domain(df_seq[df_seq["entry"] == "P05067"], window=1,
...
class TestPredictWindow:
    def test_columns(self, fitted):
        aapred, df_seq, _ = fitted
        df = aapred.predict_window(df_seq[df_seq["entry"] == "P05067"][["entry", "sequence"]],
                                   tmd_len=15, step=20)
    ...
    def test_step(self, fitted):
        aapred, df_seq, _ = fitted
        d1 = aapred.predict_window(df_seq[df_seq["entry"] == "P05067"][["entry", "sequence"]], tmd_len=15, step=5)
        d2 = aapred.predict_window(df_seq[df_seq["entry"] == "P05067"][["entry", "sequence"]], tmd_len=15, step=10)
        assert len(d1) > len(d2)

    def test_jmd_lengths(self, fitted):
        aapred, df_seq, _ = fitted
        df = aapred.predict_window(df_seq[df_seq["entry"] == "P05067"][["entry", "sequence"]],
                                   tmd_len=15, step=25, jmd_n_len=8, jmd_c_len=8)
        assert len(df) > 0

    def test_list_parts(self, fitted):
        aapred, df_seq, _ = fitted
        df = aapred.predict_window(df_seq[df_seq["entry"] == "P05067"][["entry", "sequence"]],
                                   tmd_len=15, step=25, list_parts=["tmd", "jmd_n_tmd_n", "tmd_c_jmd_c"])
        assert len(df) > 0
...
class TestPlotWindow:
    def _dw(self, fitted, step=15):
        aapred, df_seq, _ = fitted
        return aapred.predict_window(df_seq[df_seq["entry"] == "P05067"][["entry", "sequence"]],
                                     tmd_len=15, step=step)

    def test_returns_fig_ax(self, fitted):
        fig, ax = aa.AAPredPlot().window(df_window=self._dw(fitted))
        assert fig is not None and ax is not None

    def test_threshold(self, fitted):
        fig, ax = aa.AAPredPlot().window(self._dw(fitted), threshold=0.75)
    ...
    def test_entry_multi_requires_entry(self, fitted):
        ...
        dw = aapred.predict_window(two, tmd_len=15, step=30)
        with pytest.raises(ValueError):
            aa.AAPredPlot().window(dw)
        fig, ax = aa.AAPredPlot().window(dw, entry="P05067")
        assert ax is not None

    def test_list_annotations(self, fitted):
        dw = self._dw(fitted)
        tracks = [{"values": np.linspace(0, 1, len(dw)), "label": "pLDDT", "cmap": "RdYlBu"}]
        fig, ax = aa.AAPredPlot().window(dw, list_annotations=tracks)
        assert ax is not None

    def test_color_labels(self, fitted):
        fig, ax = aa.AAPredPlot().window(self._dw(fitted), color="#123456",
                                         xlabel="Pos", ylabel="P")
        assert ax.get_ylabel() == "P"

    def test_ax_figsize(self, fitted):
        fig, ax0 = plt.subplots()
        fig, ax = aa.AAPredPlot().window(self._dw(fitted), ax=ax0, figsize=(10, 4))
        assert ax is ax0
...
class TestPlotDomain:
    def _dd(self, fitted):
        aapred, df_seq, _ = fitted
        return aapred.predict_domain(df_seq[df_seq["entry"] == "P05067"], window=3)

    def test_returns_fig_ax(self, fitted):
        fig, ax = aa.AAPredPlot().domain(df_domain=self._dd(fitted))
        assert fig is not None and ax is not None

    def test_best_marked(self, fitted):
        fig, ax = aa.AAPredPlot().domain(self._dd(fitted))
        assert ax.get_legend() is not None

    def test_entry_color_labels(self, fitted):
        fig, ax = aa.AAPredPlot().domain(self._dd(fitted), entry="P05067", color="#123456",
                                         xlabel="Offset", ylabel="Score")
        assert ax.get_xlabel() == "Offset"

    def test_ax_figsize(self, fitted):
        fig, ax0 = plt.subplots()
        fig, ax = aa.AAPredPlot().domain(self._dd(fitted), ax=ax0, figsize=(6, 4))
        assert ax is ax0
