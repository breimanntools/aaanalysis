"""This is a script to test AAPredPlot.predict_group(kind="rank_scatter") — the per-protein rank scatter."""
import matplotlib
matplotlib.use("Agg")
...
def _rank_scatter(df_rank, **kwargs):
    """Call the rank_scatter kind, defaulting col_group to the fixture's 'group' column."""
    kwargs.setdefault("col_group", "group")
    return aa.AAPredPlot.predict_group(df_rank, kind="rank_scatter", **kwargs)
...
class TestPredictGroupRankScatter:
    """Normal cases for predict_group(kind='rank_scatter')."""

    def test_returns_fig_ax(self):
        fig, ax = _rank_scatter(_df())
    ...
    def test_threshold_list_draws_lines(self):
        fig, ax = _rank_scatter(_df(), thresholds=[0.5, 0.8])
        assert len(ax.get_lines()) == 2

    def test_threshold_scalar_draws_one_line(self):
        fig, ax = _rank_scatter(_df(), thresholds=0.5)
    ...
    def test_canonical_substrate_color_is_green(self):
        fig, ax = _rank_scatter(_df(), group_order=["substrate", "hold-out", "non-substrate"])
    ...
    def test_custom_dict_color_applied(self):
        fig, ax = _rank_scatter(_df(n_sub=5, n_hold=0, n_non=5),
    ...
    def test_draws_on_passed_ax(self):
        ...
        fig, ax = _rank_scatter(_df(), ax=ax0)
    ...
    def test_dict_color_partial_with_fallback(self):
        ...
        fig, ax = _rank_scatter(df, group_order=["substrate", "hold-out", "non-substrate"],
    ...
    def test_returns_fig_ax_result(self):
        # predict_group returns the FigAxResult tuple that also forwards to the axes
        res = _rank_scatter(_df())
        assert isinstance(res, ut.FigAxResult)
        fig, ax = res
        assert isinstance(ax, plt.Axes)


class TestPredictGroupRankScatterComplex:
    """Negative cases and combinations."""

    def test_col_group_required_raises(self):
        # rank_scatter needs a group column (predict_group's shared col_group defaults None)
        with pytest.raises(ValueError):
            aa.AAPredPlot.predict_group(_df(), kind="rank_scatter")
    ...
    def test_empty_df_raises(self):
        with pytest.raises(ValueError):
            _rank_scatter(_df(n_sub=0, n_hold=0, n_non=0))

    def test_group_order_missing_group_raises(self):
        with pytest.raises(ValueError):
            _rank_scatter(_df(), group_order=["substrate"])  # omits two present groups

    @pytest.mark.parametrize("bad", ["x", [0.5, "y"]])
    def test_threshold_non_numeric_raises(self, bad):
        with pytest.raises(ValueError):
            _rank_scatter(_df(), thresholds=bad)

    def test_negative_marker_size_raises(self):
        with pytest.raises(ValueError):
            _rank_scatter(_df(), marker_size=-5)

    def test_single_group(self):
        df = pd.DataFrame({"score": [0.1, 0.9, 0.5], "group": ["a", "a", "a"]})
        fig, ax = _rank_scatter(df)
    ...
    def test_negative_threshold_allowed(self):
        ...
        fig, ax = _rank_scatter(df, thresholds=-0.1)
        assert len(ax.get_lines()) == 1

    def test_bad_col_score_name_raises(self):
        with pytest.raises(ValueError):
            _rank_scatter(_df(), col_score="nope")

    def test_bad_col_group_name_raises(self):
        with pytest.raises(ValueError):
            _rank_scatter(_df(), col_group="nope")
