...
class TestObtainSamples:
    """Positive and negative tests for ap.obtain_samples(), one parameter per test."""

    # Positive tests
    def test_returns_triple(self):
        out = ap.obtain_samples(df_seq, strategy="same_protein", seed=0)
    ...
    def test_df_samples_segments_schema(self):
        df_samples, _, _ = ap.obtain_samples(df_seq, strategy="same_protein", seed=0)
    ...
    def test_strategy_parameter(self, strategy):
        df_samples, _, df_eval = ap.obtain_samples(df_seq, strategy=strategy, seed=1)
    ...
    def test_n_parameter(self, n):
        df_samples, _, df_eval = ap.obtain_samples(df_seq, strategy="same_protein", n=n, seed=2)
    ...
    def test_window_size_parameter(self, window_size):
        df_samples, _, _ = ap.obtain_samples(df_seq, strategy="same_protein",
    ...
    def test_pos_col_parameter(self):
        ...
        df_samples, _, _ = ap.obtain_samples(df, strategy="same_protein",
    ...
    def test_seed_parameter(self, seed):
        df_samples, _, _ = ap.obtain_samples(df_seq, strategy="different_protein", seed=seed)
        assert len(df_samples) > 0

    def test_max_similarity_to_test_parameter(self):
        df_samples, _, df_eval = ap.obtain_samples(
    ...
    def test_plot_parameter(self):
        df_samples, ax_on, _ = ap.obtain_samples(df_seq, strategy="same_protein",
                                                  plot=True, seed=0)
        _, ax_off, _ = ap.obtain_samples(df_seq, strategy="same_protein", plot=False, seed=0)
    ...
    def test_invalid_df_seq(self):
        with pytest.raises(ValueError):
            ap.obtain_samples("invalid", strategy="same_protein")

    def test_invalid_pos_col(self):
        with pytest.raises(ValueError, match="pos_col"):
            ap.obtain_samples(df_seq, pos_col="not_a_column", strategy="same_protein")

    def test_invalid_strategy(self):
        for strategy in ["motif_matched", "bogus", 1]:
            with pytest.raises(ValueError):
                ap.obtain_samples(df_seq, strategy=strategy)

    def test_invalid_n(self):
        for n in [0, -1, 1.5]:
            with pytest.raises(ValueError):
                ap.obtain_samples(df_seq, strategy="same_protein", n=n)

    def test_invalid_window_size(self):
        for window_size in [0, -3, "bad"]:
            with pytest.raises(ValueError):
                ap.obtain_samples(df_seq, strategy="same_protein", window_size=window_size)

    def test_invalid_max_similarity_to_test(self):
        for val in [-0.1, 1.5, "bad"]:
            with pytest.raises(ValueError):
                ap.obtain_samples(df_seq, strategy="same_protein", max_similarity_to_test=val)

    def test_invalid_seed(self):
        for seed in [-1, "bad", 1.5]:
            with pytest.raises(ValueError):
                ap.obtain_samples(df_seq, strategy="same_protein", seed=seed)

    def test_invalid_plot(self):
        with pytest.raises(ValueError):
            ap.obtain_samples(df_seq, strategy="same_protein", plot="yes")

    def test_invalid_verbose(self):
        with pytest.raises(ValueError):
            ap.obtain_samples(df_seq, strategy="same_protein", verbose="loud")

    def test_reliable_negatives_without_df_feat(self):
        with pytest.raises(ValueError, match="df_feat"):
            ap.obtain_samples(df_seq, strategy="different_protein", reliable_negatives=True)

    def test_reliable_negatives_wrong_strategy(self):
        with pytest.raises(ValueError, match="different_protein"):
            ap.obtain_samples(df_seq, strategy="same_protein",
    ...
    def test_no_positive_windows_raises(self):
        ...
        with pytest.raises(ValueError, match="positive window"):
            ap.obtain_samples(df, strategy="same_protein")
    ...
    def test_pos_col_with_nan_in_list_cell(self):
        ...
        df_samples, _, df_eval = ap.obtain_samples(df, strategy="synthetic",
...
class TestObtainSamplesComplex:
    ...
    def test_reproducible_same_seed(self):
        df_1, _, _ = ap.obtain_samples(df_seq, strategy="different_protein", n=8, seed=7)
        df_2, _, _ = ap.obtain_samples(df_seq, strategy="different_protein", n=8, seed=7)
    ...
    def test_byte_identical_negatives_to_sampler(self):
        ...
        df_samples, _, _ = ap.obtain_samples(df_seq, strategy="same_protein", n=10, seed=5)
    ...
    def test_default_n_balances(self):
        df_samples, _, df_eval = ap.obtain_samples(df_seq, strategy="same_protein", seed=0)
    ...
    def test_synthetic_strategy_tags(self):
        df_samples, _, _ = ap.obtain_samples(df_seq, strategy="synthetic", n=6, seed=1)
    ...
    def test_synthetic_reports_no_source_proteins(self):
        ...
        _, _, df_eval = ap.obtain_samples(df_seq, strategy="synthetic", n=6, seed=1)
    ...
    def test_pu_path_reproducible(self):
        ...
        df_1, _, _ = ap.obtain_samples(df_seq, **kws)
        df_2, _, _ = ap.obtain_samples(df_seq, **kws)
        assert df_1.equals(df_2)

    def test_combined_valid_parameters(self):
        df_samples, ax, df_eval = ap.obtain_samples(
    ...
    def test_plot_returns_logo_axes_per_group(self):
        ...
        df_samples, ax, _ = ap.obtain_samples(df_seq, strategy="different_protein",
    ...
    def test_combined_invalid_parameters(self):
        with pytest.raises(ValueError):
            ap.obtain_samples(df_seq, strategy="bogus", n=-5, seed="bad")

    def test_eval_leakage_respects_threshold(self):
        _, _, df_eval = ap.obtain_samples(df_seq, strategy="same_protein",
...
class TestObtainSamplesGoldenValues:
    ...
    def test_positive_window_string_and_count(self):
        df = self._tiny()
        df_samples, _, df_eval = ap.obtain_samples(df, strategy="different_protein",
                                                    window_size=5, n=1, seed=0)
    ...
    def test_balance_ratio_exact(self):
        df = self._tiny()
        _, _, df_eval = ap.obtain_samples(df, strategy="different_protein",
                                           window_size=5, n=1, seed=0)
