...
class TestPredictSamples:
    """Positive and negative tests for ap.predict_samples(), one parameter per test."""

    # Positive tests
    def test_returns_triple(self):
        result = ap.predict_samples(df_feat, df_seq, labels, models=["rf"], random_state=0, plot=False)
    ...
    def test_default_models_compared(self):
        predictors, _, df_eval = ap.predict_samples(df_feat, df_seq, labels, random_state=0)
        assert len(df_eval) == 4
        assert len(predictors) == 4

    def test_df_eval_metric_columns(self):
        _, _, df_eval = ap.predict_samples(df_feat, df_seq, labels, models=["rf"], random_state=0)
    ...
    def test_single_df_feat_named(self):
        predictors, _, df_eval = ap.predict_samples(df_feat, df_seq, labels, models=["rf"], random_state=0)
        assert list(df_eval["feature_set"].unique()) == ["features"]
        assert ("features", "rf") in predictors

    def test_list_df_feat_auto_named(self):
        _, _, df_eval = ap.predict_samples([df_feat, df_feat.head(4)], df_seq, labels,
                                            models=["rf"], random_state=0)
        assert set(df_eval["feature_set"]) == {"feat1", "feat2"}

    def test_dict_df_feat_keeps_names(self):
        predictors, _, df_eval = ap.predict_samples({"a": df_feat, "b": df_feat.head(4)}, df_seq,
    ...
    def test_models_string(self):
        _, _, df_eval = ap.predict_samples(df_feat, df_seq, labels, models=["rf", "svm"], random_state=0)
        assert set(df_eval["model"]) == {"rf", "svm"}

    def test_models_instance(self):
        predictors, _, _ = ap.predict_samples(df_feat, df_seq, labels,
    ...
    def test_models_dict_keeps_names(self):
        predictors, _, df_eval = ap.predict_samples(df_feat, df_seq, labels,
    ...
    def test_random_state_parameter(self, random_state):
        _, _, df_eval = ap.predict_samples(df_feat, df_seq, labels, models=["rf"], random_state=random_state)
    ...
    def test_n_cv_parameter(self, n_cv):
        _, _, df_eval = ap.predict_samples(df_feat, df_seq, labels, models=["rf"], n_cv=n_cv, random_state=0)
        assert np.isfinite(df_eval["balanced_accuracy_mean"]).all()

    def test_n_jobs_parameter(self):
        for n_jobs in [None, 1]:
            _, _, df_eval = ap.predict_samples(df_feat, df_seq, labels, models=["rf"],
                                                n_jobs=n_jobs, random_state=0)
            assert np.isfinite(df_eval["balanced_accuracy_mean"]).all()

    def test_verbose_parameter(self):
        for verbose in [True, False]:
            predictors, _, _ = ap.predict_samples(df_feat, df_seq, labels, models=["rf"],
                                                   verbose=verbose, random_state=0)
            assert len(predictors) == 1

    def test_is_best_exactly_one(self):
        _, _, df_eval = ap.predict_samples(df_feat, df_seq, labels, random_state=0)
        assert int(df_eval["is_best"].sum()) == 1

    def test_is_shap_ready_flags_tree_models(self):
        _, _, df_eval = ap.predict_samples(df_feat, df_seq, labels,
    ...
    def test_invalid_df_feat(self):
        with pytest.raises(ValueError):
            ap.predict_samples("invalid", df_seq, labels)
        with pytest.raises(ValueError):
            ap.predict_samples(pd.DataFrame({"not_feature": [1, 2]}), df_seq, labels)

    def test_invalid_df_seq(self):
        with pytest.raises(ValueError):
            ap.predict_samples(df_feat, "invalid", labels)

    def test_invalid_labels_length(self):
        with pytest.raises(ValueError):
            ap.predict_samples(df_feat, df_seq, labels[:-1], models=["rf"])

    def test_invalid_n_cv(self):
        for n_cv in [1, 0, -1, "invalid"]:
            with pytest.raises(ValueError):
                ap.predict_samples(df_feat, df_seq, labels, models=["rf"], n_cv=n_cv)

    def test_invalid_random_state(self):
        for random_state in [-1, "invalid", 1.5]:
            with pytest.raises(ValueError):
                ap.predict_samples(df_feat, df_seq, labels, models=["rf"], random_state=random_state)

    def test_invalid_verbose(self):
        with pytest.raises(ValueError):
            ap.predict_samples(df_feat, df_seq, labels, models=["rf"], verbose="invalid")

    def test_invalid_models_string(self):
        with pytest.raises(ValueError):
            ap.predict_samples(df_feat, df_seq, labels, models=["not_a_model"])

    def test_invalid_models_type(self):
        with pytest.raises(ValueError):
            ap.predict_samples(df_feat, df_seq, labels, models=[123])

    def test_empty_list_df_feat(self):
        with pytest.raises(ValueError):
            ap.predict_samples([], df_seq, labels, models=["rf"])

    def test_empty_models(self):
        with pytest.raises(ValueError):
            ap.predict_samples(df_feat, df_seq, labels, models=[])
...
class TestPredictSamplesComplex:
    ...
    def test_grid_shape(self):
        predictors, _, df_eval = ap.predict_samples({"a": df_feat, "b": df_feat.head(4)}, df_seq, labels,
    ...
    def test_returned_predictors_are_fitted(self):
        predictors, _, _ = ap.predict_samples(df_feat, df_seq, labels, models=["rf"], random_state=2)
    ...
    def test_reproducible_same_seed(self):
        _, _, e1 = ap.predict_samples(df_feat, df_seq, labels, models=["rf"], random_state=7)
        _, _, e2 = ap.predict_samples(df_feat, df_seq, labels, models=["rf"], random_state=7)
        assert e1.equals(e2)
    ...
    def test_n_features_matches_feature_set_size(self):
        _, _, df_eval = ap.predict_samples({"six": df_feat, "four": df_feat.head(4)}, df_seq, labels,
    ...
    def test_metrics_in_unit_range(self):
        _, _, df_eval = ap.predict_samples(df_feat, df_seq, labels, models=["rf"], random_state=1)
        for m in _METRIC_COLS:
    ...
    def test_combined_invalid_parameters(self):
        with pytest.raises(ValueError):
            ap.predict_samples(df_feat, df_seq, labels, n_cv=-5, random_state="bad")

    def test_invalid_df_feat_in_dict(self):
        with pytest.raises(ValueError):
            ap.predict_samples({"good": df_feat, "bad": pd.DataFrame({"x": [1]})}, df_seq, labels,
                                models=["rf"])

    def test_n_cv_exceeds_class_count(self):
        with pytest.raises(ValueError):
            ap.predict_samples(df_feat, df_seq, labels, models=["rf"], n_cv=999)
    ...
    def test_invalid_labels_in_grid(self):
        with pytest.raises(ValueError):
            ap.predict_samples({"a": df_feat, "b": df_feat.head(4)}, df_seq, labels[:-2], models=["rf"])
...
class TestPredictSamplesPlot:
    ...
    def test_plot_true_returns_axes(self):
        _, ax, _ = ap.predict_samples(df_feat, df_seq, labels, models=["rf", "svm"], random_state=0, plot=True)
    ...
    def test_plot_false_returns_none(self):
        _, ax, _ = ap.predict_samples(df_feat, df_seq, labels, models=["rf"], random_state=0, plot=False)
        assert ax is None

    def test_plot_does_not_change_table(self):
        _, _, e_plot = ap.predict_samples(df_feat, df_seq, labels, models=["rf"], random_state=0, plot=True)
        _, _, e_noplot = ap.predict_samples(df_feat, df_seq, labels, models=["rf"], random_state=0, plot=False)
        assert e_plot.equals(e_noplot)

    def test_figsize_dict_color_baseline(self):
        _, ax, _ = ap.predict_samples(df_feat, df_seq, labels, models=["rf"], random_state=0,
    ...
    def test_multi_feature_sets_distinct_bars(self):
        ...
        _, ax, _ = ap.predict_samples({"a": df_feat, "b": df_feat.head(4)}, df_seq, labels,
    ...
    def test_invalid_plot(self):
        with pytest.raises(ValueError):
            ap.predict_samples(df_feat, df_seq, labels, models=["rf"], plot="yes")

    def test_invalid_baseline(self):
        with pytest.raises(ValueError):
            ap.predict_samples(df_feat, df_seq, labels, models=["rf"], baseline="mid")
