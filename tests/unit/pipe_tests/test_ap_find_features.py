...
class TestFindFeatures:
    ...
    def test_returns_triple(self):
        df_feat, ax, df_eval = ap.find_features(labels, df_seq=df_seq, search="fast",
    ...
    def test_plot_false_returns_none_axes(self):
        _, ax, _ = ap.find_features(labels, df_seq=df_seq, search="fast",
                                     plot=False, random_state=0, n_jobs=1)
        assert ax is None

    def test_df_feat_importance_sorted(self):
        df_feat, _, _ = ap.find_features(labels, df_seq=df_seq, search="fast",
                                          plot=False, random_state=0, n_jobs=1)
    ...
    def test_search_fast_single_row_eval(self):
        df_feat, _, df_eval = ap.find_features(labels, df_seq=df_seq, search="fast",
                                                plot=False, random_state=0, n_jobs=1)
    ...
    def test_model_parameter(self, model):
        _, _, df_eval = ap.find_features(labels, df_seq=df_seq, search="fast",
    ...
    def test_subcategories_parameter(self):
        ...
        df_feat, _, _ = ap.find_features(labels, df_seq=df_seq, subcategories=subs, search="fast",
    ...
    def test_name_test_ref_parameters(self):
        _, ax, _ = ap.find_features(labels, df_seq=df_seq, search="fast", plot=True,
    ...
    def test_df_seq_parameter(self):
        df_feat, _, _ = ap.find_features(labels=labels, df_seq=df_seq, search="fast",
                                          plot=False, random_state=0, n_jobs=1)
        assert len(df_feat) > 0
    ...
    def test_invalid_search(self):
        for bad in ["nope", "optimization", "", 1]:
            with pytest.raises(ValueError):
                ap.find_features(labels, df_seq=df_seq, search=bad, plot=False)

    def test_invalid_df_seq(self):
        for bad in [None, pd.DataFrame({"x": [1]}), "not_a_df"]:
            with pytest.raises(ValueError):
                ap.find_features(labels, df_seq=bad, search="fast", plot=False)
    ...
    def test_invalid_cv(self):
        for bad in [1, 0, -1, "x"]:
            with pytest.raises(ValueError):
                ap.find_features(labels, df_seq=df_seq, search="fast", cv=bad, plot=False)

    def test_invalid_simplify(self):
        for bad in ["yes", 1, None]:
            with pytest.raises(ValueError):
                ap.find_features(labels, df_seq=df_seq, search="fast", simplify=bad, plot=False)

    def test_invalid_top_n(self):
        for bad in [0, -1, "x"]:
            with pytest.raises(ValueError):
                ap.find_features(labels, df_seq=df_seq, search="fast", top_n=bad, plot=False)

    def test_invalid_random_state(self):
        for bad in [-1, "x", 1.5]:
            with pytest.raises(ValueError):
                ap.find_features(labels, df_seq=df_seq, search="fast", random_state=bad, plot=False)

    def test_invalid_plot(self):
        for bad in ["yes", 1, None]:
            with pytest.raises(ValueError):
                ap.find_features(labels, df_seq=df_seq, search="fast", plot=bad)

    def test_invalid_verbose(self):
        for bad in ["yes", 1, None]:
            with pytest.raises(ValueError):
                ap.find_features(labels, df_seq=df_seq, search="fast", plot=False, verbose=bad)

    def test_invalid_kws_unknown_key(self):
        with pytest.raises(ValueError):
            ap.find_features(labels, df_seq=df_seq, search="balanced",
                              kws={"not_a_lever": 1}, plot=False)

    def test_invalid_kws_type(self):
        with pytest.raises(ValueError):
            ap.find_features(labels, df_seq=df_seq, search="balanced", kws="not_a_dict", plot=False)

    def test_invalid_subcategories_no_match(self):
        with pytest.raises(ValueError):
            ap.find_features(labels, df_seq=df_seq, subcategories=["__nonexistent__"],
...
class TestFindFeaturesComplex:
    ...
    def test_fast_byte_identical_to_explicit_chain(self):
        ...
        df_a, _, _ = ap.find_features(labels, df_seq=df_seq, search="fast",
                                       plot=False, random_state=0, n_jobs=1)
        assert df_m.equals(df_a)

    def test_reproducible_same_seed_fast(self):
        df_1, _, e_1 = ap.find_features(labels, df_seq=df_seq, search="fast",
                                         plot=False, random_state=7, n_jobs=1)
        df_2, _, e_2 = ap.find_features(labels, df_seq=df_seq, search="fast",
                                         plot=False, random_state=7, n_jobs=1)
        assert df_1.equals(df_2) and e_1.equals(e_2)

    def test_top_n_after_importance_sort(self):
        df_full, _, _ = ap.find_features(labels, df_seq=df_seq, search="fast",
                                          plot=False, random_state=0, n_jobs=1)
        df_top, _, _ = ap.find_features(labels, df_seq=df_seq, search="fast",
                                         top_n=5, plot=False, random_state=0, n_jobs=1)
        assert df_top.equals(df_full.head(5))

    @pytest.mark.slow
    def test_balanced_staged_search(self):
        df_feat, _, df_eval = ap.find_features(labels, df_seq=df_seq, search="balanced",
    ...
    def test_balanced_sweeps_n_jmd(self):
        ...
        _, _, df_eval = ap.find_features(labels, df_seq=df_seq, search="balanced", kws=SMALL,
                                          plot=False, random_state=0, n_jobs=1)
        assert "n_jmd" in df_eval.columns
    ...
    def test_balanced_winner_n_jmd_consistent(self):
        ...
        df_feat, _, df_eval = ap.find_features(labels, df_seq=df_seq, search="balanced", kws=SMALL,
                                                plot=False, random_state=0, n_jobs=1)
    ...
    def test_reproducible_same_seed_balanced_n_jmd(self):
        _, _, e1 = ap.find_features(labels, df_seq=df_seq, search="balanced", kws=SMALL,
                                     plot=False, random_state=11, n_jobs=1)
        _, _, e2 = ap.find_features(labels, df_seq=df_seq, search="balanced", kws=SMALL,
                                     plot=False, random_state=11, n_jobs=1)
    ...
    def test_reproducible_same_seed_balanced(self):
        d1, _, e1 = ap.find_features(labels, df_seq=df_seq, search="balanced", kws=SMALL,
                                      plot=False, random_state=7, n_jobs=1)
        d2, _, e2 = ap.find_features(labels, df_seq=df_seq, search="balanced", kws=SMALL,
                                      plot=False, random_state=7, n_jobs=1)
        assert d1.equals(d2) and e1.equals(e2)

    @pytest.mark.slow
    def test_exhaustive_sweeps_parts(self):
        df_feat, _, df_eval = ap.find_features(labels, df_seq=df_seq, search="exhaustive",
    ...
    def test_combined_subcategories_top_n_plot(self):
        ...
        df_feat, ax, _ = ap.find_features(labels, df_seq=df_seq, subcategories=subs,
    ...
    def test_fast_free_peptides_auto_fits_and_warns(self):
        ...
        with pytest.warns(UserWarning, match="too short"):
            df_feat, _, df_eval = ap.find_features(
    ...
    def test_balanced_ax_eval_publication_figures(self):
        from matplotlib.figure import Figure
        _, ax, _ = ap.find_features(labels, df_seq=df_seq, search="balanced",
