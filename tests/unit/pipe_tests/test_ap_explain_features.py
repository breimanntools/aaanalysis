...
def _manual_explain(samples, names, random_state=1, label_target_class=1, list_model_classes=None):
    """The explicit primitive chain ap.explain_features is supposed to mirror byte-for-byte."""
...
class TestExplainFeatures:
    """Positive and negative tests for ap.explain_features(), one parameter per test."""

    # Positive tests
    def test_returns_triple(self):
        result = ap.explain_features(df_feat.copy(), df_seq, labels, samples=0, random_state=0)
    ...
    def test_evals_slot_is_none(self):
        _, _, evals = ap.explain_features(df_feat.copy(), df_seq, labels, samples=0, random_state=0)
        assert evals is None

    def test_impact_column_added(self):
        df_shap, _, _ = ap.explain_features(df_feat.copy(), df_seq, labels, samples=entry0, random_state=0)
    ...
    def test_plot_false_returns_none_axes(self):
        df_shap, ax, _ = ap.explain_features(df_feat.copy(), df_seq, labels, samples=0, plot=False, random_state=0)
        assert ax is None
        assert any(c.startswith("feat_impact_") for c in df_shap.columns)

    def test_samples_auto_select(self):
        df_shap, _, _ = ap.explain_features(df_feat.copy(), df_seq, labels, random_state=0, plot=False)
    ...
    def test_samples_position(self):
        df_shap, _, _ = ap.explain_features(df_feat.copy(), df_seq, labels, samples=2, plot=False, random_state=0)
        entry2 = str(df_seq["entry"].iloc[2])
        assert f"feat_impact_{entry2}" in df_shap.columns

    def test_samples_entry_name(self):
        df_shap, _, _ = ap.explain_features(df_feat.copy(), df_seq, labels, samples=entry0, plot=False, random_state=0)
        assert f"feat_impact_{entry0}" in df_shap.columns

    def test_samples_list(self):
        names = [str(df_seq["entry"].iloc[i]) for i in (0, 1)]
        df_shap, _, _ = ap.explain_features(df_feat.copy(), df_seq, labels, samples=[0, 1], plot=False, random_state=0)
        for n in names:
            assert f"feat_impact_{n}" in df_shap.columns

    def test_list_model_classes_parameter(self):
        df_shap, _, _ = ap.explain_features(df_feat.copy(), df_seq, labels, samples=0,
                                             list_model_classes=[RandomForestClassifier], plot=False, random_state=0)
        assert any(c.startswith("feat_impact_") for c in df_shap.columns)

    def test_label_target_class_parameter(self):
        df_shap, _, _ = ap.explain_features(df_feat.copy(), df_seq, labels, label_target_class=0,
                                             plot=False, random_state=0)
        assert any(c.startswith("feat_impact_") for c in df_shap.columns)

    def test_name_test_ref_parameters(self):
        _, ax, _ = ap.explain_features(df_feat.copy(), df_seq, labels, samples=0,
    ...
    def test_invalid_label_target_class(self):
        for ltc in [-1, "x", 1.5]:
            with pytest.raises(ValueError):
                ap.explain_features(df_feat.copy(), df_seq, labels, label_target_class=ltc, random_state=0)

    def test_invalid_random_state(self):
        for random_state in [-1, "invalid", 1.5]:
            with pytest.raises(ValueError):
                ap.explain_features(df_feat.copy(), df_seq, labels, samples=0, random_state=random_state)

    def test_invalid_plot(self):
        with pytest.raises(ValueError):
            ap.explain_features(df_feat.copy(), df_seq, labels, samples=0, plot="yes", random_state=0)

    def test_absent_target_class_raises(self):
        # No sample of label_target_class=2 -> auto-select cannot pick a sample
        with pytest.raises(ValueError):
            ap.explain_features(df_feat.copy(), df_seq, labels, label_target_class=2, plot=False, random_state=0)
...
class TestExplainFeaturesComplex:
    ...
    def test_byte_identical_to_manual_chain(self):
        ...
        df_ap, _, _ = ap.explain_features(df_feat.copy(), df_seq, labels, samples=entry0,
                                            random_state=1, plot=False)
        col = f"feat_impact_{entry0}"
        assert np.array_equal(df_ap[col].to_numpy(), df_man[col].to_numpy())
    ...
    def test_byte_identical_with_explicit_models(self):
        ...
        df_ap, _, _ = ap.explain_features(df_feat.copy(), df_seq, labels, samples=0,
                                            list_model_classes=models, random_state=3, plot=False)
        col = f"feat_impact_{entry0}"
        assert np.array_equal(df_ap[col].to_numpy(), df_man[col].to_numpy())

    def test_random_state_reproducible(self):
        df1, _, _ = ap.explain_features(df_feat.copy(), df_seq, labels, random_state=5, plot=False)
        df2, _, _ = ap.explain_features(df_feat.copy(), df_seq, labels, random_state=5, plot=False)
    ...
    def test_does_not_mutate_input_df_feat(self):
        df_in = df_feat.copy()
        ap.explain_features(df_in, df_seq, labels, samples=0, plot=False, random_state=0)
