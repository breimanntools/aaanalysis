"""This is a script to test EmbeddingPreprocessor.build_cat()."""
import warnings
...
class TestClusterPseudoScales:
    """Positive and parameter-level negative tests for build_cat."""

    # Positive cases
    def test_returns_dataframe(self):
        df_scales = _make_pseudo_scales(D=8)
        df_cat = aa.EmbeddingPreprocessor().build_cat(
            df_scales=df_scales, cat_min_th=0.3, subcat_min_th=0.6, random_state=0,
        )
        assert isinstance(df_cat, pd.DataFrame)

    def test_shape_rows_match_D(self):
        df_scales = _make_pseudo_scales(D=12)
        df_cat = aa.EmbeddingPreprocessor().build_cat(
            df_scales=df_scales, cat_min_th=0.3, subcat_min_th=0.6, random_state=0,
        )
    ...
    def test_scale_id_matches_df_scales_columns(self):
        df_scales = _make_pseudo_scales(D=8)
        df_cat = aa.EmbeddingPreprocessor().build_cat(
            df_scales=df_scales, cat_min_th=0.3, subcat_min_th=0.6, random_state=0,
        )
        assert df_cat["scale_id"].tolist() == list(df_scales.columns)
    ...
    def test_subcat_has_at_least_as_many_clusters_as_cat(self):
        ...
        df_scales = _make_pseudo_scales(D=16)
        df_cat = aa.EmbeddingPreprocessor().build_cat(
            df_scales=df_scales, cat_min_th=0.2, subcat_min_th=0.8, random_state=0,
        )
    ...
    def test_deterministic_with_same_random_state(self):
        ...
        df1 = aa.EmbeddingPreprocessor().build_cat(
            df_scales=df_scales, cat_min_th=0.3, subcat_min_th=0.6, random_state=0,
        )
        df2 = aa.EmbeddingPreprocessor().build_cat(
            df_scales=df_scales, cat_min_th=0.3, subcat_min_th=0.6, random_state=0,
        )
    ...
    def test_accepts_any_nonneg_random_state(self, rs):
        df_scales = _make_pseudo_scales(D=6)
        df_cat = aa.EmbeddingPreprocessor().build_cat(
            df_scales=df_scales, cat_min_th=0.3, subcat_min_th=0.6, random_state=rs,
        )
    ...
    # Negative cases
    def test_invalid_df_scales_none(self):
        with pytest.raises(ValueError):
            aa.EmbeddingPreprocessor().build_cat(
                df_scales=None, cat_min_th=0.3, subcat_min_th=0.6, random_state=0,
            )

    def test_invalid_df_scales_not_dataframe(self):
        for bad in ["str", 1, np.zeros((20, 5))]:
            with pytest.raises(ValueError):
                aa.EmbeddingPreprocessor().build_cat(
                    df_scales=bad, cat_min_th=0.3, subcat_min_th=0.6, random_state=0,
                )
    ...
    def test_invalid_cat_min_th_negative(self):
        ...
        with pytest.raises(ValueError):
            aa.EmbeddingPreprocessor().build_cat(
                df_scales=df_scales, cat_min_th=-0.1, subcat_min_th=0.6, random_state=0,
            )
    ...
    def test_invalid_cat_min_th_above_one(self):
        ...
        with pytest.raises(ValueError):
            aa.EmbeddingPreprocessor().build_cat(
                df_scales=df_scales, cat_min_th=1.5, subcat_min_th=0.6, random_state=0,
            )
    ...
    def test_invalid_subcat_min_th_negative(self):
        ...
        with pytest.raises(ValueError):
            aa.EmbeddingPreprocessor().build_cat(
                df_scales=df_scales, cat_min_th=0.3, subcat_min_th=-0.1, random_state=0,
            )
    ...
    def test_invalid_subcat_min_th_above_one(self):
        ...
        with pytest.raises(ValueError):
            aa.EmbeddingPreprocessor().build_cat(
                df_scales=df_scales, cat_min_th=0.3, subcat_min_th=1.5, random_state=0,
            )
    ...
    def test_invalid_cat_min_th_geq_subcat_min_th(self):
        ...
        for cat_th, sub_th in [(0.5, 0.5), (0.7, 0.3)]:
            with pytest.raises(ValueError, match="should be <"):
                aa.EmbeddingPreprocessor().build_cat(
                    df_scales=df_scales, cat_min_th=cat_th, subcat_min_th=sub_th, random_state=0,
                )
    ...
    def test_invalid_random_state_negative(self):
        ...
        with pytest.raises(ValueError):
            aa.EmbeddingPreprocessor().build_cat(
                df_scales=df_scales, cat_min_th=0.3, subcat_min_th=0.6, random_state=-1,
            )

    def test_invalid_df_scales_too_few_dims(self):
        """D < 3 should be rejected (AAclust requires ≥3 samples to cluster)."""
        for D in [1, 2]:
            df_scales = _make_pseudo_scales(D=D)
    ...
    def test_metric_default_is_correlation(self):
        ...
        df_scales = _make_pseudo_scales(D=10)
        df_default = aa.EmbeddingPreprocessor().build_cat(
            df_scales=df_scales, cat_min_th=0.3, subcat_min_th=0.6, random_state=0,
        )
        df_explicit = aa.EmbeddingPreprocessor().build_cat(
            df_scales=df_scales, cat_min_th=0.3, subcat_min_th=0.6, random_state=0,
            metric="correlation",
        )
    ...
    # df_stds parameter (std-aware clustering)
    def test_accepts_df_stds(self):
        """Supplying df_stds enables std-aware mode and returns the standard (D, 5) shape."""
        df_scales = _make_pseudo_scales(D=10, seed=0)
        df_stds = _make_pseudo_scales(D=10, seed=1).abs()  # nonneg stds
    ...
    def test_std_aware_deterministic_with_same_random_state(self):
        df_scales = _make_pseudo_scales(D=10, seed=0)
        df_stds = _make_pseudo_scales(D=10, seed=2).abs()
        df_stds.index = df_scales.index
        df_stds.columns = df_scales.columns
        df1 = aa.EmbeddingPreprocessor().build_cat(
            df_scales=df_scales, df_stds=df_stds,
    ...
    def test_std_aware_equivalent_to_manual_concat_zscore(self):
        """Validates the math: passing (df_scales, df_stds) to std-aware mode is
        ...
        df_scales = _make_pseudo_scales(D=10, seed=0)
        df_stds = _make_pseudo_scales(D=10, seed=1).abs()
        df_stds.index = df_scales.index
        df_stds.columns = df_scales.columns
        ...
        M = df_scales.T.values  # (D, 20)
        S = df_stds.T.values    # (D, 20)
        manual = np.concatenate([_zsc(M), _zsc(S)], axis=1)  # (D, 40)
        # Mean-only path expects df_scales of shape (n_features, D), then transposes
        # internally to (D, n_features). So we pass the (40, D) frame here.
        df_manual = pd.DataFrame(
            manual.T,
            index=[f"feat_{i}" for i in range(40)],
            columns=df_scales.columns,
        )
        df_via_mean_only_on_manual = aa.EmbeddingPreprocessor().build_cat(
            df_scales=df_manual, cat_min_th=0.3, subcat_min_th=0.6, random_state=0,
        )
        df_via_std_aware = aa.EmbeddingPreprocessor().build_cat(
            df_scales=df_scales, df_stds=df_stds,
            cat_min_th=0.3, subcat_min_th=0.6, random_state=0,
        )
    ...
    def test_dims_with_similar_mean_different_std_separate_under_std_aware(self):
        ...
        df_mean_only = aa.EmbeddingPreprocessor().build_cat(
            df_scales=df_scales, cat_min_th=0.3, subcat_min_th=0.6, random_state=0,
        )
        df_std_aware = aa.EmbeddingPreprocessor().build_cat(
            df_scales=df_scales, df_stds=df_stds,
            cat_min_th=0.3, subcat_min_th=0.6, random_state=0,
        )
    ...
    # Negative cases for df_stds
    def test_invalid_df_stds_wrong_shape(self):
        df_scales = _make_pseudo_scales(D=6)
        df_stds = _make_pseudo_scales(D=8).abs()  # D mismatch
        df_stds.index = df_scales.index
    ...
    def test_invalid_df_stds_mismatched_index_or_columns(self):
        df_scales = _make_pseudo_scales(D=6)
        # Same shape but shuffled AA index
        df_stds_bad_idx = df_scales.abs().copy()
        df_stds_bad_idx.index = list(reversed(df_scales.index))
        with pytest.raises(ValueError, match="same index"):
            aa.EmbeddingPreprocessor().build_cat(
                df_scales=df_scales, df_stds=df_stds_bad_idx,
                cat_min_th=0.3, subcat_min_th=0.6, random_state=0,
            )
        # Same shape but renamed columns
        df_stds_bad_cols = df_scales.abs().copy()
        df_stds_bad_cols.columns = [f"X_{i}" for i in range(df_scales.shape[1])]
        with pytest.raises(ValueError, match="same columns"):
            aa.EmbeddingPreprocessor().build_cat(
                df_scales=df_scales, df_stds=df_stds_bad_cols,
                cat_min_th=0.3, subcat_min_th=0.6, random_state=0,
            )

    def test_invalid_df_stds_with_nan(self):
        df_scales = _make_pseudo_scales(D=6)
        df_stds = df_scales.abs().copy()
        df_stds.iloc[3, 2] = np.nan
...
class TestClusterPseudoScalesComplex:
    """Combinations and edge interactions for build_cat."""

    def test_pipeline_with_build_scales(self):
        """End-to-end: feed build_scales output directly into build_cat."""
        rng = np.random.default_rng(0)
        df_seq = pd.DataFrame({
        ...
        with warnings.catch_warnings():
            ...
            df_scales = aa.EmbeddingPreprocessor().build_scales(df_seq=df_seq, dict_num=embeddings)
        # Skip rows with NaN (AAs absent from corpus) — AAclust needs complete rows
        df_scales = df_scales.dropna()
        df_cat = aa.EmbeddingPreprocessor().build_cat(
            df_scales=df_scales, cat_min_th=0.3, subcat_min_th=0.6, random_state=0,
        )
    ...
    def test_thresholds_yield_different_partitions(self):
        ...
        df_scales = _make_pseudo_scales(D=20)
        df_cat = aa.EmbeddingPreprocessor().build_cat(
            df_scales=df_scales, cat_min_th=0.2, subcat_min_th=0.9, random_state=0,
        )
        # Subcategory should produce more clusters than category
        assert df_cat["subcategory"].nunique() > df_cat["category"].nunique()

    def test_large_D_still_works(self):
        df_scales = _make_pseudo_scales(D=64)
        df_cat = aa.EmbeddingPreprocessor().build_cat(
            df_scales=df_scales, cat_min_th=0.3, subcat_min_th=0.6, random_state=0,
        )
    ...
    def test_invalid_combined_none_inputs(self):
        with pytest.raises(ValueError):
            aa.EmbeddingPreprocessor().build_cat(
                df_scales=None, cat_min_th=-1, subcat_min_th=0.6, random_state=0,
            )
    ...
    def test_invalid_combined_reversed_thresholds(self):
        ...
        with pytest.raises(ValueError, match="should be <"):
            aa.EmbeddingPreprocessor().build_cat(
                df_scales=df_scales, cat_min_th=0.9, subcat_min_th=0.1, random_state=0,
            )

    def test_pipeline_into_cpp_run(self):
        """End-to-end: build_scales → build_cat → CPP.run.

        Verifies the (df_scales, df_cat) pair is a drop-in replacement
        ...
        with warnings.catch_warnings():
            warnings.simplefilter("ignore")
            df_scales = aa.EmbeddingPreprocessor().build_scales(
    ...
    def test_pipeline_into_cpp_run_std_aware(self):
        """End-to-end std-aware variant: build_scales(return_std=True) →
        build_cat(df_stds=...) → CPP.run."""
        rng = np.random.default_rng(0)
        df_seq = aa.load_dataset(name="DOM_GSEC", n=10)
        labels = df_seq["label"].to_list()
        D = 6
        ...
        with warnings.catch_warnings():
            warnings.simplefilter("ignore")
            df_scales, df_stds = aa.EmbeddingPreprocessor().build_scales(
                df_seq=df_seq, dict_num=embeddings, return_std=True,
            )
        df_scales = df_scales.dropna()
        df_stds = df_stds.loc[df_scales.index]  # drop the same rows
        df_cat = aa.EmbeddingPreprocessor().build_cat(
            df_scales=df_scales, df_stds=df_stds,
            cat_min_th=0.3, subcat_min_th=0.6, random_state=0,
        )
