...
class TestEncode:
    ...
    def test_stores_norm_params(self):
        ...
        ep.encode(df_seq=df_seq, embeddings=embeddings)
        assert isinstance(ep.norm_params_, dict) and ep.norm_params_["method"] == "minmax"
...
class TestEncodeComplex:
    ...
    def test_encode_then_run_num_pipeline(self):
        ...
        ep = aa.EmbeddingPreprocessor()
        dict_num = ep.encode(df_seq=df_seq, embeddings=embeddings)
    ...
    def test_methods_agree_on_shape(self):
        ...
        for m in ("minmax", "quantile", "sigmoid"):
            dn = ep.encode(df_seq=df_seq, embeddings=embeddings, method=m)
