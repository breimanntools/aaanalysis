...
class TestEncodeOneHot:
    ...
    def test_list_seq_valid(self, list_seq):
        ...
        result = seqp.encode_one_hot(list_seq=list_seq)
    ...
    def test_list_seq_invalid(self):
        ...
        with pytest.raises(ValueError):
            seqp.encode_one_hot(list_seq=None)
        with pytest.raises(ValueError):
            seqp.encode_one_hot(list_seq=[])
        with pytest.raises(ValueError):
            seqp.encode_one_hot(list_seq=dict())
        with pytest.raises(ValueError):
            seqp.encode_one_hot(list_seq=["INVALIDSEQUENCE"])
    ...
    def test_alphabet_valid(self, alphabet):
        ...
        result = seqp.encode_one_hot(list_seq=[valid_seq], alphabet=alphabet)
        assert isinstance(result, tuple)
        assert isinstance(result[0], np.ndarray)
        assert isinstance(result[1], list)
    ...
    def test_alphabet_invalid(self):
        ...
        seqp = aa.SequencePreprocessor()
        with pytest.raises(ValueError):
            seqp.encode_one_hot(list_seq=["ACDEF"], alphabet=None)
        with pytest.raises(ValueError):
            seqp.encode_one_hot(list_seq=["ACDEF"], alphabet="")
        with pytest.raises(ValueError):
            seqp.encode_one_hot(list_seq=["ACDEF"], alphabet=123)
    ...
    def test_gap_valid(self, gap):
        ...
        seqp = aa.SequencePreprocessor()
        result = seqp.encode_one_hot(list_seq=["ARND"], gap=gap)
        assert isinstance(result, tuple)
        assert isinstance(result[0], np.ndarray)
        assert isinstance(result[1], list)
    ...
    def test_gap_invalid(self):
        ...
        seqp = aa.SequencePreprocessor()
        with pytest.raises(ValueError):
            seqp.encode_one_hot(list_seq=["ACDEF"], gap=None)
        with pytest.raises(ValueError):
            seqp.encode_one_hot(list_seq=["ACDEF"], gap="")
        with pytest.raises(ValueError):
            seqp.encode_one_hot(list_seq=["ACDEF"], gap="INVALID")
    ...
    def test_pad_at_valid(self, pad_at):
        ...
        seqp = aa.SequencePreprocessor()
        result = seqp.encode_one_hot(list_seq=["ARND"], pad_at=pad_at)
        assert isinstance(result, tuple)
        assert isinstance(result[0], np.ndarray)
        assert isinstance(result[1], list)
    ...
    def test_pad_at_invalid(self):
        ...
        seqp = aa.SequencePreprocessor()
        with pytest.raises(ValueError):
            seqp.encode_one_hot(list_seq=["ACDEF"], pad_at=None)
        with pytest.raises(ValueError):
            seqp.encode_one_hot(list_seq=["ACDEF"], pad_at="")
        with pytest.raises(ValueError):
            seqp.encode_one_hot(list_seq=["ACDEF"], pad_at="INVALID")
...
class TestEncodeOneHotComplex:
    ...
    def test_valid_combination(self, list_seq, alphabet, gap, pad_at):
        ...
        result = seqp.encode_one_hot(list_seq=list_seq, alphabet=alphabet, gap=gap, pad_at=pad_at)
        assert isinstance(result, tuple)
        assert isinstance(result[0], np.ndarray)
        assert isinstance(result[1], list)
...
class TestEncodeOneHotEquivalence:
    ...
    def test_matches_reference(self, pad_at, seed):
        ...
        seqp = aa.SequencePreprocessor()
        fm, feats = seqp.encode_one_hot(list_seq=seqs, pad_at=pad_at)
    ...
    def test_invalid_combination(self, list_seq, alphabet, gap, pad_at):
        ...
        seqp = aa.SequencePreprocessor()
        with pytest.raises(ValueError):
            seqp.encode_one_hot(list_seq=list_seq, alphabet=alphabet, gap=gap, pad_at=pad_at)
