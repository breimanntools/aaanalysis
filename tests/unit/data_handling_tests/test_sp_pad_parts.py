...
class TestPadPartsInput:
    ...
    def test_df_parts_invalid_type(self):
        ...
        with pytest.raises(ValueError):
            sp.pad_parts(df_parts=["AC", "ACDEF"])
        with pytest.raises(ValueError):
            sp.pad_parts(df_parts="ACDEF")
        with pytest.raises(ValueError):
            sp.pad_parts(df_parts=None)
        with pytest.raises(ValueError):
            sp.pad_parts(df_parts=dict())

    def test_df_parts_non_string_column(self):
        """A selected column with non-string values raises a clear ValueError."""
        sp = aa.SequencePreprocessor()
        with pytest.raises(ValueError):
            sp.pad_parts(df_parts=pd.DataFrame({"tmd": [1, 2, 3]}))
    ...
    def test_df_parts_non_string_only_checked_when_selected(self):
        ...
        out = sp.pad_parts(df_parts=df, list_parts=["tmd"])
...
class TestPadPartsLength:
    ...
    def test_length_valid(self, length):
        ...
        out = sp.pad_parts(df_parts=df, length=length)
    ...
    def test_length_none_per_part_max(self):
        ...
        sp = aa.SequencePreprocessor()
        out = sp.pad_parts(df_parts=_df(), length=None)
    ...
    def test_length_explicit(self):
        ...
        sp = aa.SequencePreprocessor()
        out = sp.pad_parts(df_parts=_df(), length=6)
        assert out["tmd"].tolist() == ["AC----", "ACDEF-"]
    ...
    def test_length_too_short_raises(self):
        ...
        sp = aa.SequencePreprocessor()
        with pytest.raises(ValueError):
            sp.pad_parts(df_parts=_df(), length=3)   # 'ACDEF' (5) > 3
    ...
    def test_length_invalid(self):
        ...
        for bad in [0, -3, 2.5, "5"]:
            with pytest.raises(ValueError):
                sp.pad_parts(df_parts=_df(), length=bad)
...
class TestPadPartsGap:
    ...
    def test_gap_valid(self, gap):
        ...
        sp = aa.SequencePreprocessor()
        out = sp.pad_parts(df_parts=_df(), gap=gap)
    ...
    def test_gap_invalid(self):
        ...
        for bad in [None, "", "--"]:
            with pytest.raises(ValueError):
                sp.pad_parts(df_parts=_df(), gap=bad)
...
class TestPadPartsPadAt:
    ...
    def test_pad_at_valid(self, pad_at):
        ...
        sp = aa.SequencePreprocessor()
        out = sp.pad_parts(df_parts=_df(), pad_at=pad_at)
    ...
    def test_pad_at_invalid(self):
        ...
        for bad in [None, "", "X", "c"]:
            with pytest.raises(ValueError):
                sp.pad_parts(df_parts=_df(), pad_at=bad)
...
class TestPadPartsListParts:
    ...
    def test_list_parts_none_pads_all(self):
        ...
        sp = aa.SequencePreprocessor()
        out = sp.pad_parts(df_parts=_df(), list_parts=None)
        assert out["tmd"].tolist() == ["AC---", "ACDEF"]
        assert out["jmd"].tolist() == ["A--", "ABC"]
    ...
    def test_list_parts_single_str(self):
        ...
        sp = aa.SequencePreprocessor()
        out = sp.pad_parts(df_parts=_df(), list_parts="tmd")
        assert out["tmd"].tolist() == ["AC---", "ACDEF"]
        assert out["jmd"].tolist() == ["A", "ABC"]
    ...
    def test_list_parts_list(self):
        ...
        sp = aa.SequencePreprocessor()
        out = sp.pad_parts(df_parts=_df(), length=6, list_parts=["tmd", "jmd"])
        assert out["tmd"].tolist() == ["AC----", "ACDEF-"]
        assert out["jmd"].tolist() == ["A-----", "ABC---"]
    ...
    def test_list_parts_invalid_column(self):
        ...
        sp = aa.SequencePreprocessor()
        with pytest.raises(ValueError):
            sp.pad_parts(df_parts=_df(), list_parts=["nope"])
        with pytest.raises(ValueError):
            sp.pad_parts(df_parts=_df(), list_parts=["tmd", "nope"])
...
class TestPadPartsInvariants:
    ...
    def test_input_not_mutated(self):
        sp = aa.SequencePreprocessor()
        df = _df()
        sp.pad_parts(df_parts=df, length=8)
    ...
    def test_original_residues_preserved(self):
        ...
        sp = aa.SequencePreprocessor()
        out = sp.pad_parts(df_parts=_df(), length=8, pad_at="C")
...
class TestPadPartsBoth:
    ...
    def test_ref_both_even(self):
        ...
        sp = aa.SequencePreprocessor()
        out = sp.pad_parts(df_parts=pd.DataFrame({"tmd": ["AC"]}), length=6, pad_at="both")
    ...
    def test_ref_both_odd(self):
        ...
        out = sp.pad_parts(df_parts=pd.DataFrame({"tmd": ["AC"]}), length=7, pad_at="both")
        assert out["tmd"].tolist() == ["--AC---"]

    def test_ref_both_mixed_lengths(self):
        """Golden: per-value gap counts; 'ACDEF' (k=1) -> 0 N + 1 C."""
        sp = aa.SequencePreprocessor()
        out = sp.pad_parts(df_parts=_df(), length=6, pad_at="both")
        assert out["tmd"].tolist() == ["--AC--", "ACDEF-"]

    def test_both_zero_gap_noop(self):
        """No gaps needed -> value unchanged under 'both'."""
        sp = aa.SequencePreprocessor()
        out = sp.pad_parts(df_parts=pd.DataFrame({"tmd": ["ACDEF"]}), length=5, pad_at="both")
...
class TestPadPartsReference:
    ...
    def test_ref_c_terminal(self):
        sp = aa.SequencePreprocessor()
        out = sp.pad_parts(df_parts=_df(), length=6, pad_at="C")
        assert out["tmd"].tolist() == ["AC----", "ACDEF-"]

    def test_ref_n_terminal(self):
        sp = aa.SequencePreprocessor()
        out = sp.pad_parts(df_parts=_df(), length=6, pad_at="N")
        assert out["tmd"].tolist() == ["----AC", "-ACDEF"]

    def test_ref_length_none_per_part(self):
        sp = aa.SequencePreprocessor()
        out = sp.pad_parts(df_parts=_df())
        assert out["tmd"].tolist() == ["AC---", "ACDEF"]
        assert out["jmd"].tolist() == ["A--", "ABC"]

    def test_ref_custom_gap(self):
        sp = aa.SequencePreprocessor()
        out = sp.pad_parts(df_parts=_df(), length=6, gap="*", pad_at="C")
        assert out["tmd"].tolist() == ["AC****", "ACDEF*"]
