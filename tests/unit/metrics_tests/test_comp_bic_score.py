"""
This is a script for testing aa.comp_bic_score function
"""
import numpy as np
import pytest
...
def test_bic_known_dataset():
    ...
    bic = round(aa.comp_bic_score(X, labels), 1)
...
def test_label_mapping():
    ...
    bic_1 = aa.comp_bic_score(X, labels_1)

    # Test with shuffled labels
    labels_2 = np.array([2, 1, 2, 1])
    bic_2 = aa.comp_bic_score(X, labels_2)
...
def test_empty_input():
    ...
    with pytest.raises(ValueError):
        aa.comp_bic_score(np.array([]), np.array([]))
