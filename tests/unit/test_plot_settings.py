...
from aaanalysis import plot_settings
...
class TestPlotSettings:
    ...
    def test_adjust_further_elements(self):
        plot_settings(adjust_further_elements=True)
    ...
    def test_negative_font_scale(self):
        with pytest.raises(ValueError):
            plot_settings(font_scale=-1.0)

    def test_invalid_font(self):
        with pytest.raises(ValueError):
            plot_settings(font="InvalidFont")

    def test_invalid_fig_format(self):
        with pytest.raises(ValueError):
            plot_settings(fig_format="exe")

    def test_invalid_grid_axis(self):
        with pytest.raises(ValueError):
            plot_settings(grid_axis="z")

    def test_both_no_ticks_and_short_ticks(self):
        with pytest.warns(UserWarning):
            plot_settings(no_ticks=True, short_ticks=True)

    def test_both_no_ticks_x_and_short_ticks_x(self):
        with pytest.warns(UserWarning):
            plot_settings(no_ticks_x=True, short_ticks_x=True)

    def test_both_no_ticks_y_and_short_ticks_y(self):
        with pytest.warns(UserWarning):
            plot_settings(no_ticks_y=True, short_ticks_y=True)
...
class TestPlotSettingsComplexCases:
    ...
    def test_complex_positive_cases(self, font_scale, font, fig_format, weight_bold, adjust_only_font, adjust_further_elements, grid, no_ticks, grid_axis):
        plot_settings(font_scale=font_scale, font=font, fig_format=fig_format, weight_bold=weight_bold, adjust_only_font=adjust_only_font, adjust_further_elements=adjust_further_elements, grid=grid, no_ticks=no_ticks, grid_axis=grid_axis)
