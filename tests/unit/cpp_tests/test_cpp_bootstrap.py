...
(the CPP(n_bootstrap>0) mode wired into CPP().run() and CPP().run_num()).
"""
import numpy as np
import pandas as pd
import pytest
...
def _make_cpp(n_bootstrap=4, resample="reference", bootstrap_frac=0.8, n_freq=20,
    ...
    cpp = aa.CPP(df_parts=df_parts, split_kws=split_kws, df_scales=df_scales, verbose=False,
                 random_state=random_state, n_bootstrap=n_bootstrap, resample=resample,
                 bootstrap_frac=bootstrap_frac, n_freq=n_freq, **kwargs)
    return cpp, labels
...
class TestCPPBootstrap:
    ...
    @settings(max_examples=3, deadline=None)
    @given(n_freq=some.integers(min_value=5, max_value=25))
    def test_n_freq_positive(self, n_freq):
        cpp, labels = _make_cpp(n_bootstrap=3, n_freq=n_freq)
        df_feat = cpp.run(labels=labels, n_filter=40, n_jobs=1)
        # The stable candidate set (pre full-data filtering) is capped at n_freq.
        assert len(df_feat) <= n_freq
    ...
    @pytest.mark.parametrize("n_bootstrap", [-1, -5, 1.5, "5"])
    ...
    def test_resample_negative(self, resample):
        ...
        with pytest.raises(ValueError):
            aa.CPP(df_parts=df_parts, split_kws=split_kws, df_scales=df_scales,
                   n_bootstrap=3, resample=resample)
    ...
    def test_bootstrap_frac_negative(self, bootstrap_frac):
        ...
        with pytest.raises(ValueError):
            aa.CPP(df_parts=df_parts, split_kws=split_kws, df_scales=df_scales,
                   n_bootstrap=3, bootstrap_frac=bootstrap_frac)

    @pytest.mark.parametrize("n_freq", [0, -1, -10, 2.5])
    def test_n_freq_negative(self, n_freq):
        df_parts, labels, split_kws, df_scales = _fixture()
        with pytest.raises(ValueError):
            aa.CPP(df_parts=df_parts, split_kws=split_kws, df_scales=df_scales,
                   n_bootstrap=3, n_freq=n_freq)
...
class TestCPPBootstrapComplex:
    ...
    def test_byte_identical_when_off(self):
        ...
        cpp_off = aa.CPP(df_parts=df_parts, split_kws=split_kws, df_scales=df_scales,
                         verbose=False, random_state=42, n_bootstrap=0)
    ...
    def test_run_num_bootstrap(self):
        ...
        cpp = aa.CPP(df_parts=df_parts, df_scales=df_scales, verbose=False, random_state=1,
                     n_bootstrap=3, resample="reference", n_freq=20)
        df_feat = cpp.run_num(dict_num_parts=dict_num_parts, labels=labels, n_filter=12, n_jobs=1)
        assert COL_FREQ in df_feat.columns
        assert df_feat[COL_FREQ].between(0.0, 1.0).all()

    def test_n_freq_larger_than_candidates_returns_all_available(self):
        cpp, labels = _make_cpp(n_bootstrap=3, n_freq=100000)
        df_feat = cpp.run(labels=labels, n_filter=40, n_jobs=1)
        assert len(df_feat) >= 1
        assert COL_FREQ in df_feat.columns
...
class TestCPPBootstrapGoldenValues:
    ...
    def test_frequency_values_are_multiples_of_inverse_n_bootstrap(self):
        ...
        # Each frequency must be exactly count / n_bootstrap for an integer count in [1, n_bootstrap].
    ...
    def test_output_never_exceeds_n_filter(self):
        cpp, labels = _make_cpp(n_bootstrap=3, n_freq=100)
