...
class TestAAclustPlotMedoids:
    ...
    def test_valid_labels(self, labels):
        ...
        call_aaclust_plot_medoids(X=X, labels=labels)
    ...
    def test_valid_component_x(self, component_x):
        ...
        call_aaclust_plot_medoids(X=X, component_x=component_x)
    ...
    def test_valid_component_y(self, component_y):
        ...
        X = np.random.rand(10, 5)
        call_aaclust_plot_medoids(X=X, component_y=component_y)
    ...
    def test_valid_dot_alpha(self, dot_alpha):
        ...
        X = np.random.rand(10, 5)
        call_aaclust_plot_medoids(X=X, dot_alpha=dot_alpha)
    ...
    def test_valid_palette(self, palette):
        ...
        X = np.random.rand(10, 5)
        call_aaclust_plot_medoids(X=X, palette=palette)
    ...
    def test_invalid_X_empty(self, X):
        ...
        with pytest.raises(ValueError):
            call_aaclust_plot_medoids(X=np.array(X))
    ...
    def test_invalid_component_x(self, component_x):
        ...
        with pytest.raises(ValueError):
            aac_plot = aa.AAclustPlot()
            ax, df_components = aac_plot.medoids(X, labels=labels, component_x=component_x)
            assert isinstance(ax, plt.Axes) and isinstance(df_components, pd.DataFrame)
            plt.close()
    ...
    def test_invalid_component_y(self, component_y):
        ...
        with pytest.raises(ValueError):
            aac_plot = aa.AAclustPlot()
            ax, df_components = aac_plot.medoids(X, labels=labels, component_y=component_y)
            assert isinstance(ax, plt.Axes) and isinstance(df_components, pd.DataFrame)
            plt.close()
...
class TestAAclustPlotCenterComplex:
    ...
    def test_complex_case(self, X, labels, component_x, component_y, dot_size, palette):
        ...
        call_aaclust_plot_medoids(X=X, labels=labels, component_x=component_x, component_y=component_y,
