...
_Avoid_: sequence mutator (use SeqMut), perturbation / protein_design (legacy module names; the package is `protein_engineering`).
...
**col_dif vs col_val** (CPPPlot column pointers):
Two similarly-named but **distinct** column selectors on the `CPPPlot` family, not a rename candidate. `col_dif` (on `ranking`) names **the difference metric** — anchored to the group-level `mean_dif` (test-vs-ref, signed), drawn as diverging bars centered on 0; it is the aggregate/**global** reading. `col_val` (on `heatmap` / `feature_map`) names **the flexible cell value** that colors the heatmap — the natural home for the **local**/sample view (`feat_impact_'name'`), and it may *deviate* from the pure difference to other metrics (`abs_mean_dif`, `abs_auc`, `feat_importance`). Both default to `mean_dif` and share the sample-level `_'name'` form, which is why they look alike, but the roles differ: `col_dif` = *the difference*, `col_val` = *whichever value is shown*. A diverging difference bar cannot render an unsigned `abs_auc`, so they must not be merged.
_Avoid_: treating `col_dif` and `col_val` as the same parameter or renaming one to the other.
