...
**design / engineering**:
A cross-cutting use-case class that inverts prediction: instead of asking *what distinguishes two groups*, it asks *how a mutation moves a sequence's CPP feature profile*, and uses that to move a sequence toward a target outcome ([[AAMut]] / [[SeqMut]]). Same prediction levels, opposite direction. The #37 scope was the **measurement + minimal single-objective** layer (apply mutations, measure [[ΔCPP]], rank by magnitude, suggest the top target-shift); #57 added the **ML-guided** layer on [[SeqMut]] — an optional fitted-model mode scoring each mutation by its [[prediction shift]] (`delta_pred`) and stacking mutations via [[directed evolution]] — while multi-objective/Pareto weighting and uncertainty/active-learning selection remain deferred (#59/#60). The model-free [[ΔCPP]] is always available; the model mode is opt-in (pass a `model`). [[AAMut]] stays deterministic and model-free.
...
**SeqMut**:
The **sequence-level, CPP-aware** mutator. Requires the **position-based** [[df_seq]] (`sequence` + `tmd_start`/`tmd_stop`) and a [[df_feat]]; applies point mutations and measures the [[ΔCPP]] they induce. Six verbs: `mutate` (apply a tidy `df_mut(entry, pos, to_aa)` table), `scan` (exhaustive per-position × substitution landscape, ranked by `delta_cpp`), `suggest` (top **target-shift** mutations), `eval` (**stable-vs-disruptive** per region), `combine` (score a [[combined variant]] — several point mutations applied together), and `evolve` ([[directed evolution]] — greedily stack the best mutation each round). **Model-free by default; optionally model-aware**: constructing `SeqMut(model=...)` binds a fitted classifier so each mutation also carries its [[prediction shift]] (`delta_pred`) and `suggest`/`evolve` are guided by it. The returned table *is* the mutation history (no hidden state). It is the sole CPP-coupled class in the design module.
...
**directed evolution** ([[SeqMut]]`.evolve`):
The greedy mutation-stacking loop: each round scans the *current* (already-mutated) sequence, fixes the substitution with the best objective ([[prediction shift]] with a model, else [[target-shift]]) into the running background, and repeats for `depth` rounds, never revisiting a mutated position. Builds an interpretable multi-mutation variant without a combinatorial search; the returned trajectory carries each round's cumulative score vs the wild-type.
_Avoid_: optimization (reserved for the multi-objective #59), Pareto (no trade-off front is computed — it is single-objective greedy).

**combined variant** ([[SeqMut]]`.combine`):
A set of point mutations applied **together** to one sequence and scored once (one [[ΔCPP]] / [[prediction shift]] per variant), labelled by the `'+'`-joined single mutations (e.g. `R20K+K27P`). The explicit-design counterpart to the automated [[directed evolution]]; contrast [[SeqMut]]`.mutate`, which scores each point mutation independently against the wild-type.
