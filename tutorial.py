...
def load_data():
    """Load benchmarking dataset for sequence-based prediction tasks"""
    df = aa.load_dataset()
    list_aa = df[df["Level"] == "Amino acid"]["Dataset"].to_list()
    list_seq = df[df["Level"] == "Sequence"]["Dataset"].to_list()
    # Amino acid datasets
    for name in list_aa:
        df = aa.load_dataset(name=name)
    # Sequence datasets
    for name in list_seq:
        df = aa.load_dataset(name=name)
