...
def check_hatches(marker=None, hatch=None, list_cat=None):
    ...
    if marker and hatch:
        raise ValueError(f"'hatch' can only be applied to the default marker, not to ('{marker}'). Set it to None.")
...
def check_linestyle(linestyle=None, list_cat=None):
...
def plot_set_legend(ax: Optional[plt.Axes] = None,
    ...
    marker
        Marker for legend items. '-' is added for lines.
    marker_size
        Marker size for legend items.
    lw
        Line width for legend items. If negative, corners are rounded.
    linestyle
        Style of line. Only applied for  if marker  is '-' (line).
    edgecolor
        Edge color of legend items.
    hatch
    ...
    * :func:`matplotlib.pyplot.gca` to get the current Axes instance.
    ...
    linestyle = check_linestyle(linestyle=linestyle, list_cat=list_cat)
