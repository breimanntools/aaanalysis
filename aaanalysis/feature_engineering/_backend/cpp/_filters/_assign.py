...
def _merge_part_vals(results=None, list_parts=None):
    ...
    Skips empty-chunk results (n_jobs > n_scales → some workers see an empty
    scales slice and return ``({}, {})`` from ``_assign_scale_values_to_seq``).
    At least one chunk must be non-empty.
    """
    ...
    for part in list_parts:
        chunks = [r[0][part] for r in non_empty]
        merged_vals[part] = np.concatenate(chunks, axis=-1)
        # Lengths only depend on df_parts, not the scale chunk
        merged_lens[part] = non_empty[0][1][part]
    return merged_vals, merged_lens


def _assign_scale_values_to_seq_b(df_parts=None, dict_all_scales=None, verbose=True):
...
def assign_scale_values_to_seq(df_parts=None, df_scales=None, verbose=False, n_jobs=None,
    ...
    if _staging_shape == "B":
        return _assign_scale_values_to_seq_b(
...
def assign_dict_num_to_parts(df_seq=None, dict_num=None, list_parts=None,
    ...
    dict_part_vals : Dict[str, np.ndarray (n_samples, L_part_max, D) float64]
        ...
        ``assign_scale_values_to_seq`` so downstream stages are agnostic
        of the value source.
