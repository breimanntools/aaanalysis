...
stage: ``pre_filtering_info`` consumes the (dict_part_vals, dict_part_lens)
contract from ``_filters._assign`` and emits the same (abs_mean_dif,
...
def _pre_filtering_info(
    ...
    for split_type in split_kws:
        _abs_mean_dif, _std_test, _feat_names = _pre_filtering_info_split_type(
...
# II Main Functions
def pre_filtering_info(df_parts=None, split_kws=None, dict_part_vals=None,
    ...
    second pass by ``_filters._recompute.recompute_feature_matrix``.
...
def finalize_stats(sum_test=None, sum_sq_test=None, sum_ref=None,
    ...
    the single-pass ``pre_filtering_info`` output contract.
    """
