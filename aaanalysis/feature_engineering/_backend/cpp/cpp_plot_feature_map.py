...
def _add_importance_map_legend(ax=None, y=None, x=None, fontsize_title=None, fontsize=None):
    """"""
    ...
    second_legend = ax.legend(handles=legend_handles,
...
def plot_feature_map(df_feat=None, df_cat=None,
    ...
    # TODO adjust borderpad and y to match with rest
    y_leg = -0.12
    borderpad_leg = -1
    ...
    cbar_ax_pos = (0.5, 0, 0.2, 0.015)
    cbar_ax = fig.add_axes(cbar_ax_pos)
    _cbar_kws = dict(ticksize=fontsize_labels, label=f"Feature value\n{name_test} - {name_ref}")
    if cbar_kws is not None:
        _cbar_kws.update(cbar_kws)

    # Set cat legend arguments
    _legend_kws = dict(fontsize=fontsize_labels, fontsize_title=fontsize_labels,
                       y=y_leg, x=-0.1, borderpad=borderpad_leg)
    if legend_kws is not None:
        _legend_kws.update(legend_kws)

    # Plot feat importance bars
    plot_feat_importance_bars(ax=axes[1], df=df_feat.copy(),
                              titlesize=fontsize_annotations,
                              labelsize=fontsize_annotations,
                              top_pcp_size=fontsize_annotations - 2,
                              top_pcp_weight="bold",
                              col_imp=col_imp,
    ...
    # Add importance map
    _add_importance_map(df_feat=df_feat, df_cat=df_cat,
                        col_cat=col_cat, col_imp=col_imp,
                        ax=ax, start=start, args_len=args_len)
    _add_importance_map_legend(ax=cbar_ax, y=0, x=1.5,
                               fontsize_title=fontsize_labels,
                               fontsize=fontsize_annotations)
    # Add feature title
    add_feature_title_single_fig(y=-n_subcat/80,
                                 fontsize_title=fontsize_annotations,
                                 pad_factor=3.2)
