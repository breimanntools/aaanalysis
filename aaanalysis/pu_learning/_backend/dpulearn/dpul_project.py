...
# II Main Functions
def project_into_pca_space(X_fit=None, df_pu=None, X_new=None, method="lstsq"):
    ...
    ``X_new``. Both maps reproduce ``df_pu`` on the fit pool (when n_features >= n_samples) — exact for
    the fitted samples, an approximation (interpolation) for genuinely new points.
    """
    ...
    if method == "lstsq":
        ...
        # components: exact PCA-geometry map. Row-center by each sample's own feature mean, then the
        # min-norm linear map, which equals the fitted PCA's U @ inv(Sigma) restricted to the stored
